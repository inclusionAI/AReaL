...
The `HuggingFace Model ID` can be passed directly to `from_pretrained()` methods, and
sgl-diffusion will use the optimal default parameters when initializing and generating
videos.
...
**Note**: Wan2.2 TI2V 5B has some quality issues when performing I2V generation. We are
