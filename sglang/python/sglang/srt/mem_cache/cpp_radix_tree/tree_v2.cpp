...
static NodeHandle node2id(TreeNode *node) { return node->node_id; }
...
RadixTree::RadixTree(bool disabled, std::optional<std::size_t> host_size, std::size_t page_size,
                     std::size_t threshold)
    : m_impl(std::make_unique<Impl>(disabled, host_size.has_value(), page_size,
                                    host_size.value_or(0), threshold)) {}

RadixTree::~RadixTree() = default;

std::tuple<std::vector<at::Tensor>, std::size_t, NodeHandle, NodeHandle> RadixTree::match_prefix(
    const token_vec_t &_key) {
...
std::vector<at::Tensor> RadixTree::evict(std::size_t num_tokens) {
  ...
  while (num_evict < num_tokens && !heap.empty()) {
    ...
    if (!node->is_io_free())
      continue;  // skip nodes that are undergoing IO (i.e. indices protected)
...
RadixTree::writing_through(const token_vec_t &_key, at::Tensor value) {
  ...
  if (host_prefix_length != key.size()) {
    m_impl->create_device_node(host_node, {key.begin() + host_prefix_length, key.end()},
...
void RadixTree::reset() { m_impl->reset(); }
...
std::size_t RadixTree::evictable_size() const { return m_impl->evictable_size(); }

std::size_t RadixTree::protected_size() const { return m_impl->protected_size(); }

std::size_t RadixTree::total_size() const { return m_impl->total_size(); }
