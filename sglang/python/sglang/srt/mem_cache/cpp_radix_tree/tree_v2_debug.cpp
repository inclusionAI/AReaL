...
void RadixTree::debug_print() const { m_impl->debug_print(std::clog); }
...
void RadixTree::Impl::debug_print(std::ostream &os) const {
  ...
  for (const auto &[key, child] : *root) { stack.push_back({child.get(), root, key}); }
  ...
  while (!stack.empty()) {
    ...
    _check(key.size() == page_size && node->diff_key(key, 0) == page_size, "Key is not correct",
           nid);
    _check(depth_map.count(node) == 0, "Node is visited twice", nid);
    _check(m_node_map.count(nid) == 1, "Node is not in the map", nid);
    _check(m_node_map.at(nid) == node, "Node in the map is not the same as the one in the stack",
           nid);
    _check(!node->on_gpu() || parent->is_root() || parent->on_gpu(),
           "Node on GPU must have a GPU/root parent", nid);
    if (!node->is_io_free()) {
    ...
    if (node->on_gpu() && node->ref_count == 0) { evictable_size_real += node->length(); }
    ...
    os << std::endl;
    for (const auto &[key, child] : *node) { stack.push_back({child.get(), node, key}); }
  }
  ...
  if (visited_id.size() != m_node_map.size() - 1) {
    ...
    for (const auto &id : visited_id) { id_list += std::to_string(id) + " "; }
    id_list += "), (in map: ";
    for (const auto &[id, _] : m_node_map) { id_list += std::to_string(id) + " "; }
    id_list += ")";
  ...
  for (const auto nid : visited_id) {
    ...
    if (key.size() > kSGLANG_RADIX_CPP_DEBUG_LIMIT) {
      os << "Node " << nid << ": key is too long (" << key.size() << " tokens), skipping..."
         << std::endl;
      continue;
    }
    os << "Node " << nid << ": key = [";
