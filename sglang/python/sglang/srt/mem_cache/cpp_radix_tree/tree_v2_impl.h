...
struct RadixTree::Impl {
 public:
  Impl(bool disabled, bool use_hicache, std::size_t page_size, std::size_t host_size,
  ...
  std::vector<TreeNode *> collect_leaves() const {
    ...
    for (const auto &[_, child] : m_root) { stack.push_back(child.get()); }
    while (!stack.empty()) {
      ...
      if (node->is_leaf()) {
        ...
        for (const auto &[_, child] : *node) { stack.push_back(child.get()); }
      }
    }
    return leaves;
  }
  ...
  void lock(TreeNode *node) { return lock_ref(node, /*increment=*/true); }

  void unlock(TreeNode *node) { return lock_ref(node, /*increment=*/false); }
  ...
  std::size_t evictable_size() const { return m_evictable_size; }

  std::size_t protected_size() const { return m_protected_size; }
 ...
 private:
  ...
  token_vec_t &get_key(TreeNode *node) { return get_key(node->_unsafe_tokens()); }
