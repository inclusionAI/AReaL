...
It's recommended to reduce the memory usage by appending something like
`--mem-fraction-static 0.6` to the command above.
...
You can use `--lora-path` to specify the LoRA adapter to apply during benchmarking.
E.g.,
...
You can use `--response-answer-regex` to specify how to extract the answer from the
response string. E.g.,
...
You can use `--extra-request-body` to specify additional OpenAI request parameters.
E.g.,
...
You should use the standard instructions found in the
[dedicated profiling doc](../../docs/developer_guide/benchmark_and_profiling.md) if
running this benchmark with the profile option. We recommend using `--concurrency 1` for
consistency, which makes profiling and debugging easier.
