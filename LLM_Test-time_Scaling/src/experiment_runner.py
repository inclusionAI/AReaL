...
from .evaluation import CodeExecutor, Evaluator, LLMJudge, IMOBenchEvaluator, PRBenchEvaluator, SATBenchEvaluator, GPQAEvaluator, SimpleQAEvaluator, HLEEvaluator
...
class ExperimentRunner:
    ...
    def _load_existing_results(self, results_path: str) -> None:
        ...
        try:
            ...
            if is_code_execution:
                ...
                # Store processed results for later merging
                self.processed_results = results
                
                # Only mark problems with failed groups as needing re-run
                ...
                print(f"  Completed (all groups passed): {len(self.completed_problem_ids)}")
                ...
                processed_file = results_file.parent / f"{results_file.stem}_processed{results_file.suffix}"
                processed_data = data.copy()
                processed_data["results"] = results
                if "processing_metadata" not in processed_data:
                    processed_data["processing_metadata"] = {}
                processed_data["processing_metadata"]["early_success_marked"] = True
                processed_data["processing_metadata"]["failed_problems"] = len(failed_problem_ids)
    ...
    def _create_evaluator(self) -> Optional[Evaluator]:
        ...
        if evaluator_type == "llm_judge":
        ...
        elif evaluator_type == "simpleqa":
            # SimpleQA evaluator - LLM-based flexible answer matching
            evaluator_service = create_llm_service(
                provider=self.config.evaluation.provider,
                model_name=self.config.evaluation.model_name,
                api_key=self.config.evaluation.api_key,
            ...
            return SimpleQAEvaluator(llm_service=evaluator_service, max_concurrent=32)
        elif evaluator_type == "hle":
            # HLE evaluator - LLM-based grading for all answer types
            evaluator_service = create_llm_service(
                provider=self.config.evaluation.provider,
                model_name=self.config.evaluation.model_name,
                api_key=self.config.evaluation.api_key,
            ...
            return HLEEvaluator(llm_service=evaluator_service, max_concurrent=32)
    ...
    def _create_pipeline(self) -> ScalingPipeline:
        ...
        if self.config.reflection.strategy == "ground_truth" or self.config.reflection.strategy == "ground_truth_simple":
            ...
            reflection_kwargs["evaluator"] = self.evaluator
            reflection_kwargs["use_detailed_results"] = self.config.reflection.use_detailed_results
    ...
    async def run_single_problem(
        ...
        if self.evaluator and not is_code_execution:
            ...
            iteration_8_solutions = []  # Track iteration 8 solutions that are not correct
            
            for solution in scaling_result.all_solutions:
                solution_metadata = solution.metadata or {}
                test_results = solution_metadata.get("test_results", {})
                eval_details = solution_metadata.get("eval_details")
                iteration = solution_metadata.get("iteration", 0)
                ...
                # Track iteration 8 solutions that are not correct for re-evaluation
                if iteration == 8 and not is_correct:
                    iteration_8_solutions.append(solution)
            ...
            # Re-evaluate iteration 8 solutions that were not correct
            if iteration_8_solutions and self.evaluator:
                print(f"  Re-evaluating {len(iteration_8_solutions)} iteration 8 solutions that were not correct...")
                for solution in iteration_8_solutions:
                    ...
                    # Re-evaluate
                    reeval_result = await self.evaluator.evaluate(**eval_kwargs)
                    
                    # Extract evaluation time
                    reeval_time = 0.0
                    if reeval_result.details and isinstance(reeval_result.details, dict):
                        reeval_time = reeval_result.details.get("evaluation_time_sec", 0.0) or 0.0
                        if isinstance(reeval_time, (int, float)):
                            total_evaluation_time += reeval_time
                    
                    # Create new solution entry with iteration=9
                    new_metadata = solution.metadata.copy() if solution.metadata else {}
                    new_metadata["iteration"] = 9
                    new_metadata["reevaluated_from_iteration_8"] = True
                    new_metadata["test_results"] = {
                        "passed": reeval_result.details.get("passed", 0) if reeval_result.details else 0,
                        "total": reeval_result.details.get("total", 0) if reeval_result.details else 0,
                    }
                    new_metadata["eval_details"] = reeval_result.details
                    
                    all_evaluations.append({
                        "solution_content": solution.content,
                        "is_correct": reeval_result.is_correct,
                        "score": reeval_result.score,
                        "feedback": reeval_result.feedback,
                ...
                try:
                    ...
                    async with results_lock:
                        ...
                        await loop.run_in_executor(
                            ...
                            benchmark_name,
                            start_time,
                            existing_results + results,  # All current results
                        )
                    return result
                except Exception as e:
