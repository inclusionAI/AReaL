...
"""Run aggregation experiments on existing result files.

This script reads result files containing multiple solutions per problem,
and applies different aggregation strategies to combine them.

The script extracts all solutions with metadata.iteration == <iteration> from all result files.
...
Usage:
    python -m aggregation_scripts.imobench_aggregation_random \
        --result-files path-to-results/llm_test_time_scaling/results/imobench_rollouts_gpt-oss-120b/self_eval_sequential_4seeds_imobench_20260110_024249.json \
            path-to-results/llm_test_time_scaling/results/imobench_rollouts_gpt-oss-120b/self_eval_sequential_4seeds_trial2_imobench_20260111_151032.json \
            path-to-results/llm_test_time_scaling/results/imobench_rollouts_gpt-oss-120b/baseline_imobench_20260109_190957.json \
            path-to-results/LLM_Test-time_Scaling/results/test_time_compute/imobench_rollouts_gpt-oss-120b/self_eval_sequential_16*2_imobench_20260121_002935.json \
            path-to-results/LLM_Test-time_Scaling/results/test_time_compute/imobench_rollouts_gpt-oss-120b/self_eval_sequential_2*16_imobench_20260120_155548.json \
            path-to-results/LLM_Test-time_Scaling/results/test_time_compute/imobench_rollouts_gpt-oss-120b/self_eval_sequential_2*32_imobench_20260120_155427.json \
            path-to-results/LLM_Test-time_Scaling/results/test_time_compute/imobench_rollouts_gpt-oss-120b/self_eval_sequential_4*4_imobench_20260122_015956.json \
        --output-dir aggregation_experiments/imobench_random/64_test/2*32_auto_effort_4exps \
        --solutions-per-exp 2 \
        --num-exps 4 \
        --iteration 32 \
    ...
    python -m aggregation_scripts.imobench_aggregation_random \
        --result-files path-to-results/llm_test_time_scaling/results/imobench_rollouts_gpt-oss-120b/self_eval_sequential_4seeds_imobench_20260110_024249.json \
            path-to-results/llm_test_time_scaling/results/imobench_rollouts_gpt-oss-120b/self_eval_sequential_4seeds_trial2_imobench_20260111_151032.json \
            path-to-results/llm_test_time_scaling/results/imobench_rollouts_gpt-oss-120b/baseline_imobench_20260109_190957.json \
        --output-dir aggregation_experiments/imobench_random/2_traj_low_effort_4exps \
        --solutions-per-exp 2 \
        --num-exps 4 \
    ...
    python -m aggregation_scripts.imobench_aggregation_random \
        --result-files path-to-results/llm_test_time_scaling/results/imobench_rollouts_gpt-oss-120b/self_eval_sequential_4seeds_imobench_20260110_024249.json \
            path-to-results/llm_test_time_scaling/results/imobench_rollouts_gpt-oss-120b/self_eval_sequential_4seeds_trial2_imobench_20260111_151032.json \
        --output-dir aggregation_experiments/imobench_random/4_traj_after_self_eval \
        --solutions-per-exp 4 \
        --num-exps 4 \
        --reasoning-effort auto
    ...
    python -m aggregation_scripts.imobench_aggregation_random \
        --result-files path-to-results/llm_test_time_scaling/results/imobench_rollouts_gpt-oss-120b/self_eval_sequential_4seeds_imobench_20260110_024249.json \
            path-to-results/llm_test_time_scaling/results/imobench_rollouts_gpt-oss-120b/self_eval_sequential_4seeds_trial2_imobench_20260111_151032.json \
            path-to-results/llm_test_time_scaling/results/imobench_rollouts_gpt-oss-120b/baseline_imobench_20260109_190957.json \
        --output-dir aggregation_experiments/imobench_random/8_traj \
        --solutions-per-exp 8 \
        --num-exps 4
    
    # Multiple files (all iteration=<iteration> solutions are extracted):
...
from src.evaluation import IMOBenchEvaluator
from src.evaluation import CodeExecutor, Evaluator, LLMJudge, IMOBenchEvaluator, PRBenchEvaluator
...
def load_result_file(result_file: Path) -> Dict[str, Any]:
    ...
    benchmark = data.get("benchmark", "imobench")
...
def extract_first_n_solutions(result: Dict[str, Any], n: int = 4) -> List[Solution]:
    ...
    print("all solution count: ", len(all_solutions_data))
    solutions = []
...
def _load_existing_results_for_strategy(
    resume_file: Path, strategy_name: str, num_exps: int = 1
    ...
    Args:
        ...
        num_exps: Number of experiments expected per problem. If a problem has fewer
                  exp_results than num_exps, it will be considered incomplete.
    ...
    try:
        ...
        for result in results:
            if "error" not in result and "problem_id" in result:
                ...
                # Check experiment count consistency
                if num_exps > 1:
                    # If num_exps > 1, we expect exp_results
                    # If exp_results count is less than num_exps, consider it incomplete
                    if len(exp_results) < num_exps:
                        ...
                        continue  # Don't add to completed_problem_ids
                elif num_exps == 1:
                    # If num_exps == 1, we should NOT have exp_results (single experiment case)
                    # If exp_results exists, it means this was run with num_exps > 1, so config mismatch
                    if exp_results:
                        incomplete_problems.append(problem_id)
...
async def run_aggregation_experiment(
    ...
    """Run aggregation experiment on result file(s).
    ...
    Args:
        ...
        task_domain: Task domain for template selection (default: "math" for imobench)
    ...
    # llm config dict is result file is old, should changed to new api base
    api_base = os.getenv("OPENAI_API_BASE")
    llm_config_dict[api_base] = api_base
    ...
    llm_judge = LLMJudge(evaluator_service, benchmark="imobench")
    evaluator = IMOBenchEvaluator(llm_judge=llm_judge)
    ...
    # 5. Voting
    strategies["voting"] = VotingAggregation()
    
    strategies = {} # only pairwise
    ...
    # 7. llm voting
    # pairwise_template = prompt_manager.get_template("aggregation_llm_voting")
    # strategies["llm_voting"] = LLMVotingAggregation(
    #     llm_service=llm_service,
    #     voting_prompt_template=pairwise_template,
    #     temperature=0.0,
    #     reasoning_effort=reasoning_effort,
    # )
                ...
                if num_exps > 1:
                    ...
                    for exp_idx in range(num_exps):
                        ...
                        exp_result = {
                            ...
                            "content": aggregated_solution.content,  # Same as solution_content for consistency
                    ...
                    if aggregated_solution.metadata.get("skipped", False):
                        ...
                        # Evaluate aggregated solution for other strategies
                        eval_result = await evaluator.evaluate(
                            problem=problem,
                            solution=aggregated_solution.content,
                            ground_truth=ground_truth,
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run aggregation experiments on existing result files"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="math",
        help="Task domain for template selection (default: math for imobench)",
    )
