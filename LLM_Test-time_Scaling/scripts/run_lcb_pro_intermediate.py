"""LiveCodeBench-Pro intermediate experiment script.
...
Usage:
    python scripts/run_lcb_pro_intermediate_experiment.py --model gpt-oss-120b
"""
...
# IntermediateScalingPipeline = imobench_module.IntermediateScalingPipeline
# GenerateNFromNPipeline = imobench_module.GenerateNFromNPipeline
# SparseRefinementPipeline = imobench_module.SparseRefinementPipeline
...
async def run_intermediate_experiment(
    ...
    with setup_experiment_logging(experiment_name, log_dir) as logger:
        ...
        if service_url:
            logger.info(f"Remote Service URL: {service_url}")
        logger.info("=" * 80)
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run LiveCodeBench-Pro intermediate experiments"
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    OUTPUT_DIR = f"results/intermediate_experiments/lcb_pro_{args.model}"
    API_KEY = os.getenv("OPENAI_API_KEY")
    API_BASE = os.getenv("OPENAI_API_BASE")
    LOCAL_DATA_DIR = os.getenv("LCB_PRO_DATA_DIR", "path-to-results/llm_test_time_scaling/data/local_data/lcb_testcases/data")
    LANGUAGE = "cpp"
    ...
    print("\n" + "=" * 80)
    print("LIVECODEBENCH-PRO INTERMEDIATE EXPERIMENTS")
    print("=" * 80)
    print(f"Model: {MODEL}")
