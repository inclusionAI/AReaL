...
def test_basic_completion(
    messages: List[Dict[str, str]], model: str = MODEL_NAME, **kwargs
...
def test_gpt4o_basic():
    ...
    return test_basic_completion(messages=messages, temperature=0.7, max_tokens=150)
...
def test_gpt4o_code_generation():
    ...
    messages = [
        {
            "role": "user",
            "content": "Write a Python function to calculate the factorial of a number with proper error handling.",
        }
    ]

    return test_basic_completion(messages=messages, temperature=0.3, max_tokens=300)
...
def test_gpt4o_multi_turn():
    ...
    return test_basic_completion(messages=messages, temperature=0.7, max_tokens=100)
...
def main():
    ...
    if passed == total:
        print(
            "🎉 All tests passed! OpenAI SDK is working correctly with LiteLLM proxy."
        )
    else:
