...
class Tau2EnvironmentManager(EnvironmentManagerBase):
    def __init__(self, envs, projection_f, config):
        self.memory = SimpleMemory()
        super().__init__(envs, projection_f, config)
    ...
    def reset(self):
        text_obs, infos = self.envs.reset()
        self.tasks: list[Task] = [info['task'] for info in infos]
        self.tools: list[Tool] = infos[0]['tools']
        self.agent_policy = infos[0]['policy']
        self.memory.reset(batch_size = len(text_obs))
    ...
    def step(self, text_actions: List[str]):
        ...
        return {'text': full_text_obs, 'image': None, 'anchor': text_obs}, rewards, dones, infos
    ...
    def build_text_obs(self) -> List[str]:
        ...
        for i in range(len(self.memory)):
            ...
            full_prompt = TAU2_SYSTEM_PROMPT_SOLO.format(
                agent_instruction=agent_instruction,
                domain_policy=self.agent_policy,
                ticket=self.tasks[i].ticket
            ) + "\n"
            
            tools_prompt = "<available_tools>\n"
            tools_prompt += "Below are the available tools written in json schema:\n"
            for tool in self.tools:
                # TODO: use signature + docstring with {tool} instead of {tool.openai_schema}
                tools_prompt += f"<tool>\n{tool.openai_schema}\n</tool>\n"
            tools_prompt += "</available_tools>\n"
            ...
            if len(memory) > 0:
                ...
                steps = memory[-history_length:]
                start_index = max(1, len(memory) - history_length + 1)
                history_prompt = "<history>\n"
                history_prompt += "Below is the history of the conversation:\n"
                for j, step in enumerate(steps):
                    history_prompt += "<step>\n"
                    history_prompt += f"[Action {start_index + j}] {step['action']}\n"
                    history_prompt += f"[Observation {start_index + j}] {step['text_obs']}\n"
                    history_prompt += "</step>\n"
                history_prompt += "</history>\n"
                full_prompt += history_prompt

            full_prompt += f"Your current step is {len(memory) + 1}.\n" # TODO: add max steps
            full_prompt += "You should format your response as follows:\n"
            full_prompt += TAU2_FORMAT_INSTRUCTION
            
            postprocess_text_obs.append(full_prompt)
        return postprocess_text_obs
...
def make_envs(config):
    ...
    elif "tau2" in config.env.env_name.lower():
        from agent_system.environments.env_package.tau2 import build_tau2_envs, tau2_projection
        domain = config.env.env_name.lower().split("/")[1]
        train_split = config.env.get("train_split", "train")
        test_split = config.env.get("test_split", "test")
        print(f"using train_split: {train_split} and test_split: {test_split} for domain: {domain}")
        if domain not in ["retail", "airline", "telecom", "telecom-workflow"]:
            raise ValueError(f"Unsupported domain: {domain}")
       
        max_steps = config.env.get("max_steps", 100)
        max_train_steps = config.env.get("max_train_steps", max_steps)
        max_test_steps = config.env.get("max_test_steps", max_steps)
        
        _envs = build_tau2_envs(domain=domain, split=train_split, max_steps=max_train_steps, solo_mode=True, seed=config.env.seed, env_num=config.data.train_batch_size, group_n=group_n)
        _val_envs = build_tau2_envs(domain=domain, split=test_split, max_steps=max_test_steps, solo_mode=True, seed=config.env.seed + 1000, env_num=config.data.val_batch_size, group_n=1)
        
        projection_f = partial(tau2_projection)
        envs = Tau2EnvironmentManager(_envs, projection_f, config)
        val_envs = Tau2EnvironmentManager(_val_envs, projection_f, config)
        return envs, val_envs
    else:
...
if __name__ == "__main__":
    ...
    env_name = "tau2"
    if env_name == "tau2":
        # Test Tau2EnvironmentManager
        from agent_system.environments.env_package.tau2 import build_tau2_envs, tau2_projection
        
        env_num = 1
        group_n = 1
        time1 = time.time()
        envs = build_tau2_envs(domain="telecom", split="train", solo_mode=True, seed=1, env_num=env_num, group_n=group_n)
        projection_f = partial(tau2_projection)
        config = Namespace(env=Namespace(history_length=2))
        env_manager = Tau2EnvironmentManager(envs, projection_f, config)
        time2 = time.time()
        ...
        for i in range(3):
            obs, rewards, dones, infos = env_manager.step(["<think>xxx</think><action>get_customer_by_phone(phone_number='555-123-2002')</action>"])
            print(f"=========={i}===========")
            print(obs['text'][0])
