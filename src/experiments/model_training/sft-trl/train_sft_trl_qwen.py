...
def get_tokenizer(model_name: str, trust_remote_code: bool = True, use_fast: bool = True, chat_template_path: str | Path = None) -> AutoTokenizer:
    start_time = time.time()
    print(f"Loading tokenizer for {model_name}")
    tokenizer = AutoTokenizer.from_pretrained(model_name, trust_remote_code=trust_remote_code, use_fast=use_fast)
    ...
    print(f"Tokenizer chat_template supports assistant only loss: {check_chat_template_supports_assistant_only_loss(tokenizer.chat_template)}")
    return tokenizer


def get_model(model_name: str, torch_dtype: str="auto", device_map: str="auto", use_accelerate: bool=False) -> AutoModelForCausalLM:
    start_time = time.time()
...
def get_model_and_tokenizer(model_name, torch_dtype="auto", device_map="auto", use_accelerate=False) -> tuple[AutoModelForCausalLM, AutoTokenizer]:
...
def train(qwen_model: str = DEFAULT_QWEN_MODEL, train_dataset_path: str = DEFAULT_TRAIN_DATASET_PATH, test_dataset_path: str = DEFAULT_TEST_DATASET_PATH, output_dir: str = DEFAULT_OUTPUT_DIR, max_train_datapoints: int = DEFAULT_MAX_TRAIN_DATAPOINTS, max_test_datapoints: int = DEFAULT_MAX_TEST_DATAPOINTS, use_peft: bool = DEFAULT_USE_PEFT, patience: int = DEFAULT_PATIENCE, report_to_wandb: bool = True, run_name: str = None, use_accelerate: bool = False):
    assert os.path.exists(train_dataset_path), f"Train dataset path {train_dataset_path} does not exist."
    assert os.path.exists(test_dataset_path), f"Test dataset path {test_dataset_path} does not exist."
    ...
    model, tokenizer = get_model_and_tokenizer(model_name, torch_dtype="auto", device_map="cuda", use_accelerate=use_accelerate) # If auto, this is going to create issue with accelerate
    ...
    sft_config = SFTConfig(
        ...
        chat_template_path=QWEN25_TEMPLATE_PATH if assistant_only_loss else None, # NOTE: This is required to be able to compute assistant masks.
...
if __name__ == "__main__":
    ...
    if Path(output_dir).exists():
        print(f"Output directory {output_dir} already exists. Do you want to delete it? (y/n)")
        answer = input()
        if answer == "y":
