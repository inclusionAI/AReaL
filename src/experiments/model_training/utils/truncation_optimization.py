...
from experiments.model_training.prepare_dataset import load_as_hf_dataset, remove_none_values
from experiments.model_training.train_qwen import get_tokenizer
import pandas as pd
...
def analyze_tokens(example):
    ...
    return {
        ...
        "assistant_token_positions": [i for i, mask in enumerate(example["assistant_masks"]) if mask],
        "first_assistant_token_index": next((i for i, mask in enumerate(example["assistant_masks"]) if mask), None),
        "last_assistant_token_index": len(example["assistant_masks"]) - next((i for i, mask in enumerate(example["assistant_masks"][::-1]) if mask), None),
    }
...
def extract_system_messages(example):
    ...
    return {"system_messages": [msg for msg in example["messages"] if msg["role"] == "system"]}


def analyze_token_distribution(dataset_path, model_name, chat_template_path=None, num_samples=None, remove_system_messages=False):
    """
    ...
    dataset = dataset.map(
        lambda x: {
            **x,
            **{
                f"system_{k}": v for k, v in tokenizer.apply_chat_template(
    ...
    df = pd.DataFrame([
        {
            ...
            "num_convo_tokens": example["num_tokens"] - example["num_system_tokens"],
...
def create_assistant_token_position_distribution(df) -> pd.DataFrame:
    ...
    return pd.DataFrame(assistant_token_position_counts.items(), columns=['position', 'count'])


def create_visualizations(df, output_path="token_distribution_analysis.png", num_steps=20):
    """
    ...
    fig.suptitle('Token Distribution Analysis for Truncation Optimization', fontsize=16, fontweight='bold', y=0.98)
    ...
    ax1.axvline(mean_total, color='red', linestyle='--', linewidth=2, label=f'Mean: {mean_total:.1f}')
    ax1.axvline(mean_total + std_total, color='orange', linestyle=':', label=f'+1σ: {mean_total + std_total:.1f}')
    ax1.axvline(mean_total - std_total, color='orange', linestyle=':', label=f'-1σ: {mean_total - std_total:.1f}')
    ...
    if not assistant_pos_df.empty:
        ...
        assistant_pos_df['cumulative_tokens_lost'] = assistant_pos_df.loc[::-1, 'count'].cumsum()[::-1]
        
        # Calculate total assistant tokens for percentage calculation
        total_assistant_tokens = assistant_pos_df['count'].sum()
        assistant_pos_df['percentage_tokens_lost'] = (assistant_pos_df['cumulative_tokens_lost'] / total_assistant_tokens) * 100
        
        # Create dual-axis plot
        ax2_twin = ax2.twinx()
        
        # Plot number of tokens lost on primary y-axis
        ax2.plot(assistant_pos_df['position'], assistant_pos_df['cumulative_tokens_lost'], 
                linewidth=2.5, color='darkred', label='Tokens Lost')
        ax2.set_ylabel('Number of Assistant Tokens Lost', color='darkred', fontweight='bold')
        ax2.tick_params(axis='y', labelcolor='darkred')
        ax2.grid(True, alpha=0.2)
        
        # Plot percentage on secondary y-axis
        ax2_twin.plot(assistant_pos_df['position'], assistant_pos_df['percentage_tokens_lost'], 
                     linewidth=2.5, color='darkblue', 
                     linestyle='--', label='Percentage Lost')
        ax2_twin.set_ylabel('Percentage of Assistant Tokens Lost (%)', color='darkblue', fontweight='bold')
        ax2_twin.tick_params(axis='y', labelcolor='darkblue')
        
        ax2.set_title('Assistant Token Loss by Truncation Position', fontsize=12, fontweight='bold')
        ...
        ax2.text(0.5, 0.5, 'No assistant token position data available', 
                ha='center', va='center', transform=ax2.transAxes)
        ax2.set_title('Assistant Token Loss by Truncation Position', fontsize=12, fontweight='bold')
    
    # Plot 3: Distribution of System Tokens (bottom left)
    if 'num_system_tokens' in df.columns:
        sns.histplot(data=df, x='num_system_tokens', bins=15, ax=ax3, kde=True, color='blue')
        ...
        ax3.axvline(mean_system, color='red', linestyle='--', linewidth=1.5, label=f'μ: {mean_system:.0f}')
        ax3.axvline(mean_system + std_system, color='orange', linestyle=':', alpha=0.7, label=f'±σ: {std_system:.0f}')
        ax3.axvline(mean_system - std_system, color='orange', linestyle=':', alpha=0.7)
        ax3.legend(loc='upper right', fontsize=8)
        ax3.grid(True, alpha=0.2)
    else:
        ax3.text(0.5, 0.5, 'System token data not available', 
                ha='center', va='center', transform=ax3.transAxes)
        ax3.set_title('Distribution of System Tokens', fontsize=11, fontweight='bold')
    
    # Plot 4: Distribution of Assistant Tokens (bottom middle)
    sns.histplot(data=df, x='num_assistant_tokens', bins=15, ax=ax4, kde=True, color='orange')
    ...
    ax4.axvline(mean_assistant, color='red', linestyle='--', linewidth=1.5, label=f'μ: {mean_assistant:.0f}')
    ax4.axvline(mean_assistant + std_assistant, color='darkred', linestyle=':', alpha=0.7, label=f'±σ: {std_assistant:.0f}')
    ax4.axvline(mean_assistant - std_assistant, color='darkred', linestyle=':', alpha=0.7)
    ax4.legend(loc='upper right', fontsize=8)
    ax4.grid(True, alpha=0.2)
    
    # Plot 5: Distribution of Conversation Tokens (bottom right)
    if 'num_convo_tokens' in df.columns:
        sns.histplot(data=df, x='num_convo_tokens', bins=15, ax=ax5, kde=True, color='green')
        ax5.set_title('Distribution of Conversation Tokens', fontsize=11, fontweight='bold')
        ...
        ax5.axvline(mean_convo, color='red', linestyle='--', linewidth=1.5, label=f'μ: {mean_convo:.0f}')
        ax5.axvline(mean_convo + std_convo, color='darkgreen', linestyle=':', alpha=0.7, label=f'±σ: {std_convo:.0f}')
        ax5.axvline(mean_convo - std_convo, color='darkgreen', linestyle=':', alpha=0.7)
        ax5.legend(loc='upper right', fontsize=8)
        ax5.grid(True, alpha=0.2)
    else:
        ax5.text(0.5, 0.5, 'Conversation token data not available', 
...
def print_analysis_summary(df):
    ...
    print(f"Total tokens - Mean: {df['num_tokens'].mean():.1f}, Std: {df['num_tokens'].std():.1f}, Max: {df['num_tokens'].max()}")
    print(f"Assistant tokens - Mean: {df['num_assistant_tokens'].mean():.1f}, Std: {df['num_assistant_tokens'].std():.1f}, Max: {df['num_assistant_tokens'].max()}")
    print(f"First assistant index - Mean: {df['first_assistant_token_index'].mean():.1f}, Std: {df['first_assistant_token_index'].std():.1f}")
    print(f"Last assistant index - Mean: {df['last_assistant_token_index'].mean():.1f}, Std: {df['last_assistant_token_index'].std():.1f}")
    print(f"Assistant token range: {df['first_assistant_token_index'].min():.0f} to {df['last_assistant_token_index'].max():.0f}")
...
Examples:
  ...
  python truncation_optimization.py --dataset data/train.jsonl --model Qwen/Qwen2.5-0.5B-instruct --step-size 50
    ...
    parser.add_argument(
        "--model", 
        required=True,
        help="Model name for tokenization (required)"
    )
    
    parser.add_argument(
        "--chat-template",
        help="Path to chat template file (optional)"
    )
