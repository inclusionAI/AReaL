...
python train_sft_trl.py \
  ...
  --output_dir "/home/ubuntu/victor-north-tx/tau2-bench-private/src/experiments/model_training/data/Qwen2.5-0.5B-instruct-sft-full-tau2-assistant-only-loss" \
  ...
  --assistant_only_loss \
  --report_to "wandb" \
  --run_name "Qwen2.5-0.5B-instruct-sft-full-tau2-assistant-only-loss" \
  --learning_rate 8e-5 \
  --num_train_epochs 3 \
