...
from tau2_train.data_model.message import AssistantMessage, Message, ToolCall, UserMessage
from tau2_train.data_model.simulation import DBCheck, EnvAssertionCheck, RewardInfo
from tau2_train.data_model.tasks import RewardType, Task
from tau2_train.environment.environment import Environment
from .evaluator_base import EvaluatorBase

def aggregate_payment_history(environment):
    reservations = environment.tools.db.reservations
    for res_id in reservations:
        
        payment_history = reservations[res_id].payment_history
        total_price = 0
        for x in payment_history:
            total_price += x.amount
        
        if len(payment_history) > 0:
            new_payment_history = [
                type(payment_history[0])(
                    payment_id="total",
                    amount=total_price
                )
            ]
            environment.tools.db.reservations[res_id].payment_history = new_payment_history
...
class EnvironmentEvaluator(EvaluatorBase):
    ...
    def calculate_reward(
        ...
        if process_payment_history:
            aggregate_payment_history(gold_environment)
            aggregate_payment_history(predicted_environment)

        # predicted_environment.tools.db.reservations = dict(sorted(
        #     predicted_environment.tools.db.reservations.items()
        # ))
