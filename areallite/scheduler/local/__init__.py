...
class LocalScheduler(Scheduler):
    ...
    def create_workers(self, scheduler_config):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                worker_infos.append(future.result())
    ...
    def wait_workers(self, total_timeout: float = 60.0) -> bool:
        ...
        logging.info(
            f"Waiting for all {len(self.worker_map)} workers to be ready (total timeout: {total_timeout}s)..."
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            future_to_worker = {
                executor.submit(
                    wait_for_port, ip, port, timeout=total_timeout
                ): worker_id
            ...
            if not_done:
                logging.error(
                    f"❌ Global timeout of {total_timeout}s reached. The following workers did not complete:"
                )
