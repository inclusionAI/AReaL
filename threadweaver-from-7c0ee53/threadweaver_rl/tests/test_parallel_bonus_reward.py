...
def _sample_v3_extra_infos():
    return [
        {
            "correct": True,
            "total_num_tokens": 30,
            "num_tokens_in_the_longest_thread": 10,
            "num_subtask_tokens": 5,
            "num_trial_tokens": 2,
        },
        {
            "correct": True,
            "total_num_tokens": 40,
            "num_tokens_in_the_longest_thread": 20,
            "num_subtask_tokens": 20,
            "num_trial_tokens": 0,
        },
        {
            "correct": False,
            "total_num_tokens": 40,
            "num_tokens_in_the_longest_thread": 15,
            "num_subtask_tokens": 15,
            "num_trial_tokens": 15,
        },
        {
            "correct": True,
            "total_num_tokens": 80,
            "num_tokens_in_the_longest_thread": 30,
            "num_subtask_tokens": 0,
            "num_trial_tokens": 60,
        },
    ]
...
def test_v3_reward_matches_requested_group_formula():
    scores, extra_infos = local_reward_manager._apply_groupwise_parallel_bonus(
        _sample_scores(),
        _sample_v3_extra_infos(),
        np.array(["g1", "g1", "g2", "g2"], dtype=object),
        _v3_cfg(),
    )

    expected = [1.5, -1.5, -0.5, 0.5]
    expected_length_z = [1.0, -1.0, 1.0, -1.0]
    expected_correct_parallel_z = [1.0, -1.0, -1.0, 1.0]

    for i, expected_reward in enumerate(expected):
        assert math.isclose(scores[i]["reward"], expected_reward, abs_tol=1e-6)
        assert math.isclose(extra_infos[i]["parallel_rewardv3_reward"], expected_reward, abs_tol=1e-6)
        assert math.isclose(extra_infos[i]["parallel_rewardv3_length_z"], expected_length_z[i], abs_tol=1e-6)
        assert math.isclose(
            extra_infos[i]["parallel_rewardv3_correctness_parallel_z"],
            expected_correct_parallel_z[i],
            abs_tol=1e-6,
        )

    assert math.isclose(extra_infos[0]["parallel_rewardv3_subtask_fraction"], 1.0, abs_tol=1e-6)
    assert math.isclose(extra_infos[0]["parallel_rewardv3_trial_fraction"], 0.2, abs_tol=1e-6)
    assert math.isclose(extra_infos[3]["parallel_rewardv3_trial_fraction"], 1.0, abs_tol=1e-6)
    assert extra_infos[0]["parallel_reward_version"] == "v3"


def test_server_and_local_v3_helpers_match():
    local_scores, local_infos = local_reward_manager._apply_groupwise_parallel_bonus(
        _sample_scores(),
        _sample_v3_extra_infos(),
        np.array(["g1", "g1", "g2", "g2"], dtype=object),
...
def test_reward_manager_writes_v3_reward_to_last_valid_token(monkeypatch):
    canned_results = [
        (
            0,
            {"reward": 1.0, "second_reward": 0.0},
    ...
    manager = local_reward_manager.RewardManager(tokenizer=None, num_examine=0, config=_v3_cfg())
    data = FakeData()
    result = manager(data, return_dict=True)
    ...
    assert reward_tensor.shape == torch.Size([2, 3])
    assert torch.allclose(reward_tensor[0], torch.tensor([0.0, 1.5, 0.0]))
    assert torch.allclose(reward_tensor[1], torch.tensor([0.0, 0.0, -1.5]))
    assert extra_info["parallel_rewardv3_reward"] == [pytest.approx(1.5), pytest.approx(-1.5)]
