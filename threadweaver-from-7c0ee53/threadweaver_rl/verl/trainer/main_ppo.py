...
except ImportError:  # backwards compatibility if optional module is absent
    RewardManager = None

try:
    from verl.trainer.reward_manager_with_server import RewardManagerWithServer
except ImportError:
    RewardManagerWithServer = None
...
class TaskRunner:
    ...
    def run(self, config):
        ...
        if reward_manager_type == "reward_manager":
            if RewardManager is None:
                raise ImportError(
                    "RewardManager module is not available. Please ensure verl.trainer.reward_manager is installed."
                )
            reward_fn = RewardManager(tokenizer=tokenizer, num_examine=0, config=reward_manager_config)
            val_reward_fn = RewardManager(tokenizer=tokenizer, num_examine=1, config=reward_manager_config)
        elif reward_manager_type == "reward_manager_with_server":
            if RewardManagerWithServer is None:
                raise ImportError(
                    "RewardManagerWithServer module is not available. Please ensure verl.trainer.reward_manager_with_server is installed."
                )
            reward_fn = RewardManagerWithServer(
                tokenizer=tokenizer,
                num_examine=0,
                config=reward_manager_config,
                **server_kwargs,
            )
            val_reward_fn = RewardManagerWithServer(
                tokenizer=tokenizer,
                num_examine=1,
                config=reward_manager_config,
                **server_kwargs,
