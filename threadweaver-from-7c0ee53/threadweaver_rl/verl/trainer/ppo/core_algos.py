...
def compute_grpo_outcome_advantage(
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape is (bs, response_length)
    """
    # Optional threshold to zero advantages if all rewards in group >= threshold
    zero_thr = None
    if config is not None:
        zero_thr = config.get("grpo_zero_adv_if_all_ge_le", None)

    scores = compute_grpo_outcome_normalized_scores(
...
def compute_grpo_outcome_normalized_scores(
    token_level_rewards: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    norm_adv_by_std_in_grpo: bool = True,
    ...
    """Compute normalized scalar GRPO scores per trajectory without expanding over tokens.

    If `zero_adv_if_all_ge_le` is provided (float), for any group where all
    sequence-level rewards are >= this threshold or <= this threshold, the resulting per-sample
    scores in that group are set to 0 (i.e., zero advantage).
    """
    ...
    with torch.no_grad():
        ...
        # Detect groups to zero out if enabled
        id2zero_out: dict[Any, bool] = {}
        if zero_adv_if_all_ge_le is not None:
            thr = torch.as_tensor(zero_adv_if_all_ge_le, device=scores.device, dtype=scores.dtype)
            for idx in id2score:
                vals = id2score[idx]
                # Evaluate both conditions across the whole group
                all_ge = True
                all_le = True
                for v in vals:
                    ge = bool((v >= thr).item())
                    le = bool((v <= thr).item())
                    all_ge = all_ge and ge
                    all_le = all_le and le
                    # optional early exit if both already false
                    if not all_ge and not all_le:
                        break
                id2zero_out[idx] = all_ge or all_le
        for idx in id2score:
        ...
        for i in range(bsz):
            # If this group's rewards are all >= threshold, zero the advantage
            if id2zero_out.get(index[i], False):
                scores[i] = torch.tensor(0.0, device=scores.device, dtype=scores.dtype)
            else:
