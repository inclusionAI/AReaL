...
def _get_group_ids(uid_values, n: int) -> list[str]:
    ...
    if len(uid_list) != n:
        return [str(i) for i in range(n)]
    return [str(u) for u in uid_list]


def _safe_positive_denominator(k_value: float, length_value: float, eps: float) -> float:
    if k_value <= eps or length_value <= eps:
        return float("inf")
    return max(k_value * length_value, eps)


def _apply_groupwise_reward_v3(
    scores: list[dict],
    extra_infos: list[dict],
    uid_values,
    config: Dict[str, Any],
) -> tuple[list[dict], list[dict]]:
    alpha = _cfg_float(config, "alpha", 0.0)
    k_subtask = _cfg_float(config, "k_subtask", 1.0)
    k_trial = _cfg_float(config, "k_trial", 1.0)
    beta_subtask = _cfg_float(config, "beta_subtask", 0.0)
    beta_trial = _cfg_float(config, "beta_trial", 0.0)
    ...
    group_ids = _get_group_ids(uid_values, n)
    lengths = np.array(
        [_safe_float((extra_infos[i] or {}).get("num_tokens_in_the_longest_thread", 0.0)) for i in range(n)],
        dtype=np.float32,
    )
    total_tokens = np.array(
        [_safe_float((extra_infos[i] or {}).get("total_num_tokens", 0.0)) for i in range(n)],
        dtype=np.float32,
    )
    num_subtask_tokens = np.array(
        [
            _safe_float(
                (extra_infos[i] or {}).get(
                    "num_subtask_tokens",
                    _safe_float((extra_infos[i] or {}).get("subtask_ratio", 0.0)) * float(total_tokens[i]),
                )
            )
            for i in range(n)
        ],
    ...
    num_trial_tokens = np.array(
        [
            _safe_float(
                (extra_infos[i] or {}).get(
                    "num_trial_tokens",
                    _safe_float((extra_infos[i] or {}).get("trial_ratio", 0.0)) * float(total_tokens[i]),
                )
            )
            for i in range(n)
        ],
    ...
    subtask_fraction = np.zeros(n, dtype=np.float32)
    trial_fraction = np.zeros(n, dtype=np.float32)
    for i in range(n):
        subtask_denominator = _safe_positive_denominator(k_subtask, float(lengths[i]), eps)
        trial_denominator = _safe_positive_denominator(k_trial, float(lengths[i]), eps)
        subtask_fraction[i] = min(num_subtask_tokens[i] / subtask_denominator, 1.0)
        trial_fraction[i] = min(num_trial_tokens[i] / trial_denominator, 1.0)

    correct_indicator = np.array(
        [1.0 if bool((extra_infos[i] or {}).get("correct", False)) else 0.0 for i in range(n)],
        dtype=np.float32,
    )
    correctness_parallel_value = correct_indicator * (
        1.0 + beta_subtask * subtask_fraction + beta_trial * trial_fraction
    )
    length_z = _groupwise_zscore(-lengths, group_ids, eps=eps)
    correctness_parallel_z = _groupwise_zscore(correctness_parallel_value, group_ids, eps=eps)
    reward_v3 = alpha * length_z + correctness_parallel_z
    ...
    for i in range(n):
        score = (
            ...
            else {"reward": _safe_float(scores[i], 0.0), "second_reward": 0.0}
        )
        score = dict(score)
        base_reward = _safe_float(score.get("reward", 0.0), 0.0)
        info = dict(extra_infos[i] or {})
        shaped_reward = float(reward_v3[i])
        score["reward"] = shaped_reward
        shaped_scores.append(score)

        info["parallel_reward_version"] = "v3"
        info["parallel_rewardv3_alpha"] = alpha
        info["parallel_rewardv3_k_subtask"] = k_subtask
        info["parallel_rewardv3_k_trial"] = k_trial
        info["parallel_rewardv3_beta_subtask"] = beta_subtask
        info["parallel_rewardv3_beta_trial"] = beta_trial
        info["parallel_rewardv3_subtask_fraction"] = float(subtask_fraction[i])
        info["parallel_rewardv3_trial_fraction"] = float(trial_fraction[i])
        info["parallel_rewardv3_length_z"] = float(length_z[i])
        info["parallel_rewardv3_correctness_parallel_value"] = float(correctness_parallel_value[i])
        info["parallel_rewardv3_correctness_parallel_z"] = float(correctness_parallel_z[i])
        info["parallel_rewardv3_reward"] = shaped_reward
        info["reward_before_parallel_rewardv3"] = base_reward
        info["reward_after_parallel_rewardv3"] = shaped_reward
...
def _apply_groupwise_parallel_bonus(
    ...
    if _cfg_str(config, "version", "").lower() == "v3":
        return _apply_groupwise_reward_v3(scores, extra_infos, uid_values, config)

    beta_1 = _cfg_float(
        config, "subtask_beta", _cfg_float(config, "subtask_reward_beta", 0.0)
    )
    beta_2 = _cfg_float(
        config, "trial_beta", _cfg_float(config, "trial_reward_beta", 0.0)
    )
    beta_3 = _cfg_float(
        config,
        "parallel_ratio_beta",
        _cfg_float(config, "parallel_ratio_reward_beta", 0.0),
    )
    ...
    subtask_ratio = np.array(
        [_safe_float((extra_infos[i] or {}).get("subtask_ratio", 0.0)) for i in range(n)],
        dtype=np.float32,
    )
    trial_ratio = np.array(
        [_safe_float((extra_infos[i] or {}).get("trial_ratio", 0.0)) for i in range(n)],
        dtype=np.float32,
    )
    parallel_ratio = np.array(
        [_safe_float((extra_infos[i] or {}).get("parallel_ratio", 0.0)) for i in range(n)],
        dtype=np.float32,
    )
    ...
    def __call__(self, data: DataProto, return_dict: bool = False, correctness_as_reward: bool = False):
        ...
        for idx, (i, _, valid_response_length, _, _) in enumerate(results):
            score = (
                ...
                else {"reward": _safe_float(scores[idx]), "second_reward": 0.0}
            )
