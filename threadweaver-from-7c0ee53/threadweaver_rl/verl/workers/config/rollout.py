...
class AgentLoopConfig(BaseConfig):
    num_workers: int = 8
    # Verbosity for experimental agent loops. 0=silent, >=1=enable debug prints
    verbose: int = 1
    agent_loop_config_path: Optional[str] = None
    enable_parallel_branching: bool = False
...
class RolloutConfig(BaseConfig):
    ...
    # Whether to return flattened per-call sequences in rollout outputs.
    # This is not in agent because we always set it to False during validation
    agent_return_expanded_sequences: bool = False
    # Whether to include base (pre-expanded) tensors alongside flattened sequences.
    agent_return_base_tensors_with_expanded: bool = False
