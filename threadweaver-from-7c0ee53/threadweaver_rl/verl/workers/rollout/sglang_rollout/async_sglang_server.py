...
class SGLangHttpServer:
    ...
    async def generate(
        ...
        if return_logprob:
            ...
            log_probs = [log_prob for log_prob, _ in pairs]
            token_ids = [token_id for _, token_id in pairs]
        else:
            token_ids = list(output["output_ids"])
            log_probs = None
        generated_blocks = [{"type": "sequential", "content": list(token_ids)}] if token_ids else []
        return TokenOutput(token_ids=token_ids, log_probs=log_probs, generated_blocks=generated_blocks)
