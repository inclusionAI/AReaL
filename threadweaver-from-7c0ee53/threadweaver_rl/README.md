...
Set `reward_model.config.version=v3` to use the grouped reward/advantage variant:
```
A_i =
    alpha * z(-L_i)
  + z(1_correct_i * (
...
```
`L_i` is `num_tokens_in_the_longest_thread`; `N_subtask_i` and `N_trial_i`
are the token counts in subtask/trial parallel blocks. The v3 parameters are
`alpha`, `k_subtask`, `k_trial`, `beta_subtask`, and `beta_trial`.
For GRPO runs, set `algorithm.norm_adv_by_std_in_grpo=False` if you want to
preserve the v3 scale directly.
