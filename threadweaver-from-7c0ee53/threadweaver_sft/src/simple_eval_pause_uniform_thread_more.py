...
parser.add_argument(
    "--uniform-thread-token-limit",
    type=int,
    default=None,
    help="If set, each parallel thread generation call uses the same max_new_tokens cap.",
)
...
if (
    args.uniform_thread_token_limit is not None
    and args.uniform_thread_token_limit < 0
):
    raise ValueError(
        "--uniform-thread-token-limit must be a non-negative integer when set."
    )
if args.pause_at_longest_thread_tokens is not None:
    warnings.warn(
        "--pause-at-longest-thread-tokens is ignored in simple_eval_pause_uniform_thread.py. "
        "Use --uniform-thread-token-limit instead.",
        UserWarning,
    )
...
PARALLEL_TAG_PATTERN = re.compile(r"</?parallel>", re.IGNORECASE)
THINK_CONTINUATION_SUFFIX = "</think>\n\nTo solve the problem,"


def _has_unclosed_parallel_block(text: str) -> bool:
    if not text:
        return False

    parallel_start_id = (
        special_token_ids.get("<Parallel>") if special_token_ids is not None else None
    )
    parallel_end_id = (
        special_token_ids.get("</Parallel>") if special_token_ids is not None else None
    )
    thread_start_id = (
        special_token_ids.get("<Thread>") if special_token_ids is not None else None
    )
    thread_end_id = (
        special_token_ids.get("</Thread>") if special_token_ids is not None else None
    )
    ...
    if (
        parallel_start_id is not None
        and parallel_end_id is not None
        and thread_start_id is not None
        and thread_end_id is not None
    ):
        ...
        for tok in token_ids:
            if not in_parallel:
                if tok == parallel_start_id:
                    in_parallel = True
                    in_thread = False
                continue

            if tok == parallel_end_id and not in_thread:
                in_parallel = False
                continue
            if tok == thread_start_id and not in_thread:
                in_thread = True
                continue
            if tok == thread_end_id and in_thread:
                in_thread = False
                continue
        return in_parallel

    depth = 0
    for m in PARALLEL_TAG_PATTERN.finditer(text):
...
def _append_pause_close_tags_if_missing(text: str) -> str:
    if THINK_END_PATTERN.search(text):
        return text
    rebuilt = text
    if not rebuilt.endswith("\n"):
        rebuilt += "\n"
    if _has_unclosed_parallel_block(rebuilt):
        rebuilt += "</Parallel>\n"
    return rebuilt + THINK_CONTINUATION_SUFFIX
...
def _apply_hard_prethink_limit(
    ...
    if truncated and not suffix_from_close:
        rebuilt = _append_pause_close_tags_if_missing(capped_pre_think)
    ...
    if not suffix_from_close and pre_think_tokens >= limit:
        rebuilt = _append_pause_close_tags_if_missing(generated_text)
        full_text = _continue_generation_from_prompt(
            prompt_text + rebuilt, continue_stop_tokens_ids
        )
        return full_text[len(prompt_text):] if full_text.startswith(prompt_text) else full_text
...
def branching_generate(
    ...
    def _append_think_close_if_missing(text: str) -> str:
        return _append_pause_close_tags_if_missing(text)
    ...
    def _generate_branch(
        outlines_full: str, num: str, _unused_longest_thread_tokens_before_round: int
    ):
        ...
        capped_by_uniform_thread_limit = False
        if args.uniform_thread_token_limit is not None:
            if args.uniform_thread_token_limit <= 0:
                if verbose:
                    print(
                        colored(
                            f"Skipping branch {num}: uniform thread token limit is "
                            f"{args.uniform_thread_token_limit}.",
                            "yellow",
                        )
                    )
                return thread_end, True
            per_branch_max_new_tokens = min(
                per_branch_max_new_tokens, args.uniform_thread_token_limit
            )
            capped_by_uniform_thread_limit = (
                per_branch_max_new_tokens == args.uniform_thread_token_limit
            )
        ...
        hit_uniform_thread_limit = (
            capped_by_uniform_thread_limit and finish_reason == "length"
        )
        return branch_gen, hit_uniform_thread_limit
    ...
    try:
        while True:
            ...
            branches_gen = {}
            branches_hit_uniform_limit = {}
            longest_thread_tokens_before_round = _get_longest_thread_tokens(outlines_full)
            if args.uniform_thread_token_limit is not None and verbose:
                print(
                    colored(
                        "Uniform thread token limit enabled: "
                        f"{args.uniform_thread_token_limit} tokens per thread.",
                        "blue",
                    )
                )
            ...
            try:
                ...
                for future in concurrent.futures.as_completed(futures):
                    num = futures[future]
                    branch_gen, hit_uniform_limit = future.result()
                    branches_gen[num] = branch_gen
                    branches_hit_uniform_limit[num] = hit_uniform_limit
                    if verbose:
                        print(
                            colored(
            ...
            end_seq = False
            paused_by_uniform_thread_limit = False
            for i, num in enumerate(outline_nums):
                ...
                if not thread_content.endswith(thread_end):
                    ...
                    if branches_hit_uniform_limit.get(num, False):
                        paused_by_uniform_thread_limit = True
                    else:
                        end_seq = True
            ...
            if any(branches_hit_uniform_limit.values()):
                paused_by_uniform_thread_limit = True
            ...
            if paused_by_uniform_thread_limit:
                if verbose:
                    print(
                        colored(
                            "At least one branch hit the uniform thread token limit. "
                            "All threads were closed and merged; appending </think> and continuing generation.",
                            "yellow",
                        )
                    )
...
uniform_thread_suffix = (
    f"_threadlimit{args.uniform_thread_token_limit}"
    if args.uniform_thread_token_limit is not None
    else "_threadlimitnone"
)
file_stem = f"{_sanitize_name(data_type)}_{n_samples}{split_suffix}{uniform_thread_suffix}"
...
metrics_payload = {
    ...
    "uniform_thread_token_limit": args.uniform_thread_token_limit,
...
with open(report_md_file, "w") as f:
    f.write("# Simple Eval Uniform Thread Limit Report\n\n")
    ...
    f.write(f"- uniform_thread_token_limit: {args.uniform_thread_token_limit}\n\n")
    f.write("## Accuracy\n\n")
