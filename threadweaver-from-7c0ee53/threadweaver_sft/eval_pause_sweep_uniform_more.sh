...
usage() {
  echo "Usage: $0 <checkpoint_path> [--data-type|-d <parquet_path_or_dataset_key>] [-n|--n-samples <num>] [--bfloat16] [--verbose <level>] [extra args for simple_eval_pause_uniform_thread.py]"
...
EVAL_SCRIPT="${SCRIPT_DIR}/src/simple_eval_pause_uniform_thread_more.py"
...
echo "Running uniform-thread-limit sweep for checkpoint: ${CHECKPOINT_PATH}"
echo "Uniform thread token limits: ${TOKEN_LIMITS[*]}"
...
for limit in "${TOKEN_LIMITS[@]}"; do
  echo "=============================="
  echo "Evaluating uniform thread token limit: ${limit}"
  echo "=============================="
  CMD=(python "${EVAL_SCRIPT}" \
    ...
    -n "${N_SAMPLES}" \
    --uniform-thread-token-limit "${limit}")
...
for path in metrics_files:
    ...
    if payload_model_path:
        ...
        if payload_model_path != target_model_path:
            continue
    uniform_thread_limit = payload.get("uniform_thread_token_limit")
    if uniform_thread_limit not in token_limits:
        continue
    prev = latest_by_limit.get(uniform_thread_limit)
    if prev is None or os.path.getmtime(path) > os.path.getmtime(prev["path"]):
        latest_by_limit[uniform_thread_limit] = {"path": path, "data": payload}

missing = [limit for limit in token_limits if limit not in latest_by_limit]
if missing:
    raise SystemExit(f"Missing metrics files for uniform thread token limits: {missing}")
...
csv_path = os.path.join(results_root, "uniform_thread_limit_sweep_report.csv")
md_path = os.path.join(results_root, "uniform_thread_limit_sweep_report.md")

with open(csv_path, "w", newline="", encoding="utf-8") as f:
    writer = csv.DictWriter(
        f,
        fieldnames=[
            "uniform_thread_limit",
            "pass@1",
            pass_n_key,
...
with open(md_path, "w", encoding="utf-8") as f:
    f.write(f"# Uniform Thread Limit Sweep Report\n\n")
    f.write(f"- generated_at: {datetime.now().isoformat(timespec='seconds')}\n")
    f.write(f"- checkpoint: {checkpoint_path}\n")
    f.write(f"- results_root: {results_root}\n\n")
    f.write("| uniform_thread_limit | pass@1 | " + pass_n_key + " | avg_num_tokens_in_the_longest_thread |\n")
    f.write("|---:|---:|---:|---:|\n")
    for row in rows:
        f.write(
            f"| {row['uniform_thread_limit']} | {row['pass@1']:.6f} | "
            f"{row[pass_n_key]:.6f} | {row['avg_num_tokens_in_the_longest_thread']:.2f} |\n"
        )
    f.write("\n## Metrics Files\n\n")
    for row in rows:
        f.write(f"- {row['uniform_thread_limit']}: `{row['metrics_file']}`\n")

print("\nUniform thread limit sweep summary:")
print(f"{'thread_limit':>12}  {'pass@1':>10}  {pass_n_key:>10}  {'avg_longest':>12}")
for row in rows:
    print(
        f"{row['uniform_thread_limit']:>12}  {row['pass@1']:>10.6f}  {row[pass_n_key]:>10.6f}  "
