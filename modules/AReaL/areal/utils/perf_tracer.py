...
class SessionTraceEvent(str, Enum):
    ...
    TRAJ_END = "traj_end"
    AGENT_GENERATE_START = "agent_generate_start"
    AGENT_GENERATE_END = "agent_generate_end"
    USER_GENERATE_START = "user_generate_start"
    USER_GENERATE_END = "user_generate_end"
...
class SessionRecord:
    ...
    def default_phase_configs(cls) -> tuple[PhaseSpec, ...]:
        return (
            ...
            PhaseSpec(
                name="traj",
                start_event=SessionTraceEvent.TRAJ_START,
                end_event=SessionTraceEvent.TRAJ_END,
                allow_multiple=True,
            ),
            PhaseSpec(
                name="user_generate",
                start_event=SessionTraceEvent.USER_GENERATE_START,
                end_event=SessionTraceEvent.USER_GENERATE_END,
                allow_multiple=True,
            ),
            PhaseSpec(
                name="agent_generate",
                start_event=SessionTraceEvent.AGENT_GENERATE_START,
                end_event=SessionTraceEvent.AGENT_GENERATE_END,
                allow_multiple=True,
            ),
        )
    ...
    @staticmethod
    def _compute_traj_time(record: SessionRecord) -> float | None:
        return record._phase_total_duration("traj")
    
    @staticmethod
    def _compute_user_generate_time(record: SessionRecord) -> float | None:
        return record._phase_total_duration("user_generate")

    @staticmethod
    def _compute_agent_generate_time(record: SessionRecord) -> float | None:
        return record._phase_total_duration("agent_generate")
    ...
    def default_field_specs(cls) -> tuple[FieldSpec, ...]:
        return (
            ...
            FieldSpec(
                compute=cls._compute_traj_time,
                key="traj_s",
                omit_if_none=True,
            ),
            FieldSpec(
                compute=cls._compute_user_generate_time,
                key="user_generate_s",
                omit_if_none=True,
            ),
            FieldSpec(
                compute=cls._compute_agent_generate_time,
                key="agent_generate_s",
                omit_if_none=True,
            ),
        )
...
_SESSION_TRACE_METHOD_TO_EVENT: dict[str, SessionTraceEvent] = {
    ...
    "mark_traj_start": SessionTraceEvent.TRAJ_START,
    "mark_traj_end": SessionTraceEvent.TRAJ_END,
    "mark_user_generate_start": SessionTraceEvent.USER_GENERATE_START,
    "mark_user_generate_end": SessionTraceEvent.USER_GENERATE_END,
    "mark_agent_generate_start": SessionTraceEvent.AGENT_GENERATE_START,
    "mark_agent_generate_end": SessionTraceEvent.AGENT_GENERATE_END,
}
