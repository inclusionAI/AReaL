...
Example usage for multi-GPU Data Parallel inference with ThinkMorphDP.
Supports multiple model instances per GPU for increased throughput.
"""
...
def evaluate_with_dp(
    ...
    Each GPU can run multiple model instances concurrently for increased throughput.
    Supports full interleaved text-image generation (visual thinking).
    ...
    Args:
        ...
        models_per_gpu: Number of model instances per GPU (default 1, max ~4 for 140GB GPU)
    """
    ...
    print(f"Initializing ThinkMorphDP with {num_gpus} GPUs, {models_per_gpu} model(s) per GPU...")
    dp = ThinkMorphDP(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="ThinkMorph Multi-GPU DP Inference")
