...
class GeoEditEnvironmentManager(EnvironmentManagerBase):
    """Environment manager for GeoEdit vision tool-calling tasks.

    Integrates geo_edit's VisionQATask with verl-agent's training loop.
    Parsing of LLM text output is built-in as the projection function.
    """

    import re as _re

    _ACTION_RE = _re.compile(r"<action>(.*?)</action>", _re.DOTALL | _re.IGNORECASE)
    _ANSWER_RE = _re.compile(r"<answer>(.*?)</answer>", _re.DOTALL | _re.IGNORECASE)

    def __init__(self, envs, config):
        self.memory = SimpleMemory()
        super().__init__(envs, self._parse_text_actions, config)

    def _parse_text_actions(self, text_actions):
        """Built-in projection: parse LLM text into structured actions.

        Parses:
          <action>{"name":"..","arguments":{..}}</action> -> ToolCall
          <answer>...</answer> -> final answer

        Returns:
            (parsed_actions, valids) matching verl-agent's projection_f signature.
        """
        import json
        ...
        for text in text_actions:
            action_matches = self._ACTION_RE.findall(text)
            answer_matches = self._ANSWER_RE.findall(text)

            tool_calls = []
            for body in action_matches:
                try:
            ...
            if tool_calls:
                parsed.append({"type": "tool_call", "tool_calls": tool_calls, "answer_text": ""})
                valids.append(1)
            elif answer_text:
                parsed.append({"type": "answer", "tool_calls": [], "answer_text": answer_text})
                valids.append(1)
            else:
                parsed.append({"type": "none", "tool_calls": [], "answer_text": ""})
                valids.append(0)

        return parsed, valids

    def reset(self, kwargs):
        """Reset with task data from kwargs (env_kwargs from parquet).

        kwargs should contain: task_ids, task_prompts, task_answers,
        task_image_paths, task_types.
        """
        text_obs, infos = self.envs.reset(kwargs=kwargs)
        self.tasks = text_obs  # task prompts
        self.memory.reset(batch_size=len(text_obs))

        observations = {
            "text": self.build_text_obs(text_obs, init=True),
            "image": self.envs.get_images(),
            "anchor": None,
        }
        return observations, infos
    ...
    def step(self, text_actions: List[str]):
        ...
        for a in actions:
            if a["type"] == "tool_call":
                ...
                action_summaries.append(f"Tool calls: {', '.join(names)}")
            elif a["type"] == "answer":
                action_summaries.append(f"Answer: {a['answer_text']}")
            else:
                action_summaries.append("(no action)")
        self.memory.store({"text_obs": text_obs, "action": action_summaries})

        for i, info in enumerate(infos):
            info["is_action_valid"] = to_numpy(valids[i])
            info["tool_calling"] = actions[i]["type"] == "tool_call"
    ...
    def build_text_obs(self, text_obs: List[str], init: bool = False) -> List[str]:
        """Build text observations with system prompt, tools, and history."""
        from geo_edit.prompts.system_prompts import TOOL_CALL_SYSTEM_PROMPT
        from agent_system.environments.prompts.geo_edit import (
            GEO_EDIT_TEMPLATE_NO_HIS,
            GEO_EDIT_TEMPLATE,
        )

        tool_defs = self.envs.get_tool_declarations_text()
        postprocess_text_obs = []
        ...
        for i in range(len(text_obs)):
            if init or self.config.env.history_length <= 0:
                obs = GEO_EDIT_TEMPLATE_NO_HIS.format(
                    system_prompt=TOOL_CALL_SYSTEM_PROMPT.strip(),
                    tool_definitions=tool_defs,
                    task_prompt=self.tasks[i],
                )
            else:
                obs = GEO_EDIT_TEMPLATE.format(
                    system_prompt=TOOL_CALL_SYSTEM_PROMPT.strip(),
                    tool_definitions=tool_defs,
                    task_prompt=self.tasks[i],
                    history_length=valid_lens[i],
                    history=memory_contexts[i],
                    tool_result=text_obs[i],
                )
            postprocess_text_obs.append(obs)
...
def make_envs(config):
    ...
    if "search" in config.env.env_name.lower():
        ...
        return envs, val_envs
    elif "geo_edit" in config.env.env_name.lower():
        from agent_system.environments.env_package.geo_edit import build_geo_edit_envs
        geo_edit_config = OmegaConf.to_container(config.env.geo_edit, resolve=True)
        _envs = build_geo_edit_envs(seed=config.env.seed, env_num=config.data.train_batch_size, env_config=geo_edit_config)
        _val_envs = build_geo_edit_envs(seed=config.env.seed + 1000, env_num=config.data.val_batch_size, env_config=geo_edit_config)
        envs = GeoEditEnvironmentManager(_envs, config)
        val_envs = GeoEditEnvironmentManager(_val_envs, config)
        return envs, val_envs
    else:
