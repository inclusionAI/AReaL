...
class SGLangVisionQATask(VisionQATask):
    """Vision QA task for SGLang OpenAI-compatible chat completions with tool calls."""
    ...
    def parse_action(self, step: int, action: Any, extra_info: Dict[str, int | float | str | None]):
        ...
        if not action.choices:
            raise ValueError("SGLang response contained no choices.")
        ...
        if not tool_calls and not output_text:
            logger.error("SGLang response content: %s", content)
