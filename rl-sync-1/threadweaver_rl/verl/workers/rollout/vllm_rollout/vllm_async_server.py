...
class vLLMHttpServer:
    ...
    def __init__(
        ...
        if self.config.max_model_len is None:
            print(f"Setting max_model_len to {self.config.prompt_length + self.config.response_length}")
            self.config.max_model_len = self.config.prompt_length + self.config.response_length
        else:
            print(f"max_model_len is {self.config.max_model_len}")
    ...
    async def generate(
        ...
        output_obj = final_res.outputs[0]
        token_ids = list(output_obj.token_ids)
        log_probs = None
        if sampling_params.logprobs is not None:
            log_probs = [logprobs[token_ids[i]].logprob for i, logprobs in enumerate(output_obj.logprobs)]

        matched_stop = None
        if stop_token_ids and token_ids:
            last_token = token_ids[-1]
            if last_token in stop_token_ids:
                matched_stop = last_token

        finish_reason = getattr(output_obj, "finish_reason", None)
        ...
        return TokenOutput(
            ...
            generated_blocks=[{"type": "sequential", "content": list(token_ids)}] if token_ids else [],
        )
