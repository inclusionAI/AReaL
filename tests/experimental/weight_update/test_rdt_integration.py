...
pytestmark = pytest.mark.skipif(
    not torch.cuda.is_available(), reason="RDT tests require CUDA GPU (NIXL transport)"
)
...
def _run_rdt_weight_transfer_test(n_gpus: int, test_type: str, output: str):
    """Run RDT distributed test via torchrun subprocess."""
    ...
    proc = subprocess.Popen(
        [
            ...
            f"--master_port={port}",
            "tests/experimental/weight_update/torchrun/run_rdt_weight_transfer.py",
...
# ---------------------------------------------------------------------------
# Distributed RDT tests via torchrun
# ---------------------------------------------------------------------------


@pytest.mark.multi_gpu
@pytest.mark.slow
def test_rdt_weight_transfer_lifecycle_2gpu(tmp_path_factory):
    """Test full RDT weight transfer lifecycle."""
    if current_platform.device_count() < 2:
        pytest.skip("This test requires 2 GPUs")
    output = tmp_path_factory.mktemp("test_output") / "rdt_lifecycle.out"
    _run_rdt_weight_transfer_test(2, "rdt_weight_transfer_lifecycle", str(output))


@pytest.mark.multi_gpu
@pytest.mark.slow
@pytest.mark.parametrize("n_gpus", [4])
def test_rdt_weight_transfer_lifecycle_4gpu(n_gpus, tmp_path_factory):
    """Test RDT weight transfer with 4 GPUs."""
    if current_platform.device_count() < n_gpus:
        pytest.skip(f"This test requires {n_gpus} GPUs")
    output = tmp_path_factory.mktemp("test_output") / f"rdt_lifecycle_{n_gpus}gpu.out"
    _run_rdt_weight_transfer_test(n_gpus, "rdt_weight_transfer_lifecycle", str(output))


# ---------------------------------------------------------------------------
# E2E RDT weight update: real FSDPEngine + SGLang server + gateway
# ---------------------------------------------------------------------------
...
def _make_truncated_moe_model(tmp_path, num_layers: int = 4) -> str:
    """Create truncated MoE model with reduced layers for testing."""
    import glob
    import json
    import shutil
...
def _make_local_scheduler(tmp_path, name: str, gpu_devices: list[int]):
    ...
    return LocalScheduler(
        ...
        experiment_name=f"test-rdt-{name}",
        trial_name="t0",
...
def _validate_weight_update_correctness(
    ...
    for i, url in enumerate(train_worker_urls):
        ...
        resp = httpx.post(
            f"{url}/rdt/debug/get_parameters",
    ...
    resp = httpx.post(
        f"{inf_worker_url}/rdt/debug/get_parameters",
...
def test_rdt_fsdp_e2e_weight_update(n_gpus, tmp_path_factory):
    """Full round trip: FSDPEngine → weight-update gateway → SGLang (RDT mode).

    Requires Ray cluster with NIXL tensor transport support.
    """
    import httpx
    ...
    n_half = n_gpus // 2
    tmp = tmp_path_factory.mktemp("rdt_e2e")
    model_path = _get_test_model_path()

    scheduler = _make_local_scheduler(tmp, "rdt_e2e", gpu_devices=list(range(n_gpus)))

    # IW: SGLang with RDT backend
    inf_config = InferenceEngineConfig(
        ...
        scheduling_spec=(
            SchedulingSpec(
                ...
                env_vars={"AREAL_WEIGHT_UPDATE_BACKEND": "rdt"},
            ),
        ),
        ...
        scheduling_spec=(
            SchedulingSpec(
                ...
                env_vars=dict(
                    NCCL_CUMEM_ENABLE="0",
                    NCCL_NVLS_ENABLE="0",
                    AREAL_WEIGHT_UPDATE_BACKEND="rdt",
                ),
            ),
        ),
    )
    ...
    try:
        ...
        # Randomize IW weights
        for url in inf_worker_urls:
            resp = httpx.post(f"{url}/rdt/debug/randomize_parameters", timeout=120.0)
        ...
        # -- 6. Validate TW ↔ IW parameter equality --
...
# ---------------------------------------------------------------------------
# E2E RDT weight update: MegatronEngine + SGLang server + gateway
# ---------------------------------------------------------------------------


def _validate_weight_update_correctness_megatron_rdt(
    train_worker_urls: list[str],
    inf_worker_url: str,
    param_dir,
    tag: str = "megatron",
    ...
    """Fetch params from both sides via HTTP and compare bitwise (Megatron RDT)."""
...
def _run_megatron_rdt_e2e(
    *,
    ...
    """Run MegatronEngine E2E weight update test with RDT backend."""
    import httpx
    ...
    # TW: Megatron with RDT backend
    train_config = TrainEngineConfig(
        backend=backend,
        experiment_name=f"test-rdt-{tag}",
        trial_name="t0",
        path=model_path,
    ...
    try:
        ...
        # Randomize IW weights (RDT endpoint)
        for url in inf_worker_urls:
            resp = httpx.post(f"{url}/rdt/debug/randomize_parameters", timeout=120.0)
            assert resp.status_code == 200, f"randomize_parameters failed: {resp.text}"
        ...
        _validate_weight_update_correctness_megatron_rdt(
            train_worker_urls=train_worker_urls,
            inf_worker_url=inf_worker_urls[0],
            param_dir=tmp,
            tag=tag,
...
@pytest.mark.sglang
@pytest.mark.parametrize("n_gpus", [2, 4, 8], ids=["2gpu", "4gpu", "8gpu"])
def test_rdt_megatron_dp_e2e_weight_update(n_gpus, tmp_path_factory):
    """Full round trip: MegatronEngine (pure DP) → gateway → SGLang (RDT mode).
    ...
    n_half = n_gpus // 2
    _run_megatron_rdt_e2e(
        n_gpus=n_gpus,
        backend=f"megatron:d{n_half}",
        pair_name="test_rdt_megatron_dp_e2e",
        tag="megatron_dp_e2e",
...
def test_rdt_megatron_dp_tp_e2e_weight_update(n_gpus, tp_size, tmp_path_factory):
    """Full round trip: MegatronEngine (DP+TP) → gateway → SGLang (RDT mode).

    TP ranks within a DP group each hold the same full parameter after
    all_gather_param. dp_replicated=True tells RDT only one rank per group
    ...
    _run_megatron_rdt_e2e(
        ...
        pair_name=f"test_rdt_megatron_dp{dp_size}tp{tp_size}",
...
def test_rdt_megatron_pp_e2e_weight_update(n_gpus, pp_size, tmp_path_factory):
    """Full round trip: MegatronEngine (pure PP) → gateway → SGLang (RDT mode).
    ...
    _run_megatron_rdt_e2e(
        ...
        pair_name=f"test_rdt_megatron_pp{pp_size}",
...
def test_rdt_megatron_dp_pp_e2e_weight_update(
    n_gpus, dp_size, pp_size, tmp_path_factory
):
    """Full round trip: MegatronEngine (DP+PP) → gateway → SGLang (RDT mode).
    ...
    _run_megatron_rdt_e2e(
        ...
        pair_name=f"test_rdt_megatron_dp{dp_size}pp{pp_size}",
...
def test_rdt_megatron_pp_tp_e2e_weight_update(
    n_gpus, pp_size, tp_size, tmp_path_factory
):
    """Full round trip: MegatronEngine (PP+TP) → gateway → SGLang (RDT mode).
    ...
    _run_megatron_rdt_e2e(
        ...
        pair_name=f"test_rdt_megatron_pp{pp_size}tp{tp_size}",
...
def test_rdt_megatron_cp_e2e_weight_update(n_gpus, cp_size, tmp_path_factory):
    """Full round trip: MegatronEngine (pure CP) → gateway → SGLang (RDT mode).

    CP splits the sequence across ranks for attention but all CP ranks hold
    identical parameters. dp_replicated=True tells RDT only one CP rank per
    group needs to send.
    ...
    _run_megatron_rdt_e2e(
        ...
        pair_name=f"test_rdt_megatron_cp{cp_size}",
...
def test_rdt_megatron_dp_cp_e2e_weight_update(
    n_gpus, dp_size, cp_size, tmp_path_factory
):
    """Full round trip: MegatronEngine (DP+CP hybrid) → gateway → SGLang (RDT mode).
    ...
    _run_megatron_rdt_e2e(
        ...
        pair_name=f"test_rdt_megatron_dp{dp_size}cp{cp_size}",
...
# ---------------------------------------------------------------------------
# Megatron RDT E2E tests - EP (Expert Parallelism)
# ---------------------------------------------------------------------------
...
def test_rdt_megatron_ep_e2e_weight_update(n_gpus, ep_size, tmp_path_factory):
    """Full round trip: MegatronEngine (EP) → gateway → SGLang (RDT mode).
    ...
    _run_megatron_rdt_e2e(
        ...
        pair_name=f"test_rdt_megatron_ep{ep_size}",
...
def test_rdt_megatron_dp_ep_e2e_weight_update(
    n_gpus, dp_size, ep_size, tmp_path_factory
):
    """Full round trip: MegatronEngine (DP+EP hybrid) → gateway → SGLang (RDT mode).
    ...
    _run_megatron_rdt_e2e(
        ...
        pair_name=f"test_rdt_megatron_dp{dp_size}ep{ep_size}",
