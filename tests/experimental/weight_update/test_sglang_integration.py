...
"""Integration tests for AReaL's custom SGLang server with /awex/* and /rdt/* endpoints.

Requires GPU. Marked @pytest.mark.slow and @pytest.mark.sglang to exclude
from default CI. Run manually:
    uv run pytest tests/experimental/weight_update/test_sglang_integration.py -v -s
"""
...
def sglang_server():
    """Launch AReaL's custom SGLang server with AWEX backend.
    ...
    env = os.environ.copy()
    env["AREAL_WEIGHT_UPDATE_BACKEND"] = "awex"
...
@pytest.fixture(scope="module")
def sglang_rdt_server():
    """Launch AReaL's custom SGLang server with RDT backend.

    Sets AREAL_WEIGHT_UPDATE_BACKEND=rdt to register /rdt/* endpoints.
    """
    port = find_free_ports(1)[0]
    model_path = _get_test_model_path()

    env = os.environ.copy()
    env["AREAL_WEIGHT_UPDATE_BACKEND"] = "rdt"
...
class TestAwexEndpointsRegistered:
    """Test AWEX weight update endpoints on SGLang server."""
...
class TestRdtEndpointsRegistered:
    """Test RDT weight update endpoints on SGLang server."""

    def test_report_parallelism_endpoint_exists(self, sglang_rdt_server):
        """GET /rdt/report_parallelism should return parallelism info."""
        base_url, _ = sglang_rdt_server
        resp = httpx.get(f"{base_url}/rdt/report_parallelism", timeout=30.0)
        assert resp.status_code == 200
        data = resp.json()
        assert "world_size" in data

    def test_report_weight_meta_endpoint_exists(self, sglang_rdt_server):
        """POST /rdt/report_weight_meta should return weight metadata."""
        base_url, _ = sglang_rdt_server
        resp = httpx.post(f"{base_url}/rdt/report_weight_meta", timeout=60.0)
        assert resp.status_code == 200
        data = resp.json()
        assert data.get("status") == "ok"
        assert "meta" in data

    def test_report_parallelism_returns_valid_parallelism(self, sglang_rdt_server):
        """world_size must be a positive integer (>= 1)."""
        base_url, _ = sglang_rdt_server
        resp = httpx.get(f"{base_url}/rdt/report_parallelism", timeout=30.0)
        assert resp.status_code == 200
