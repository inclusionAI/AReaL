...
The original script as well as the part from the original script used in this script are under Apache License 2.0 https://github.com/rllm-org/rllm/blob/main/LICENSE
"""
...
def get_parallel_stats(model_response_token_ids: List[int], special_token_ids: dict) -> dict:
    """
...
# ============================================================================
# Main Script for Computing Stats from Parquet File
# ============================================================================

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compute average parallel stats from a parquet file")
    parser.add_argument(
        "--parquet-file",
        type=str,
    ...
    parquet_file = os.path.expanduser(args.parquet_file)
    ...
    if args.n_samples is not None:
        ...
        print(f"Processing first {args.n_samples} samples")
    ...
    # Process each row
    print(f"\nComputing parallel stats for field: {args.text_field}")
    all_stats = []
    ...
    # Compute averages
    print("\n" + "="*60)
    print("AVERAGE PARALLEL EXECUTION METRICS")
    print("="*60)
    ...
    for metric in metrics_to_average:
        ...
        if "ratio" in metric:
            print(f"{metric:35s}: {avg_value:.4f} ± {std_value:.4f}")
        elif "count" in metric or "tokens" in metric:
            print(f"{metric:35s}: {avg_value:.2f} ± {std_value:.2f}")
        else:
            print(f"{metric:35s}: {avg_value:.2f} ± {std_value:.2f}")

    # Count how many have parallel blocks
    num_with_parallel = sum(1 for stats in all_stats if stats["with_parallel"])
    pct_with_parallel = (num_with_parallel / len(all_stats)) * 100
    print(f"\n{'Samples with <Parallel> blocks':35s}: {num_with_parallel}/{len(all_stats)} ({pct_with_parallel:.1f}%)")
