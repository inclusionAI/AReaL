...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Length penalty diagnostics (if provided by rollout)
    if "length_penalty_scores" in batch.batch:
        lp_scores = batch.batch["length_penalty_scores"]
        # Index the last generated token for each row
        last_pos = (response_mask.sum(dim=1) - 1).clamp(min=0)
        row_idx = torch.arange(lp_scores.size(0), device=lp_scores.device)
        lp_last = lp_scores[row_idx, last_pos]

        # Exclude aborted samples
        lp_last_non_aborted = lp_last[non_aborted_mask]
        if lp_last_non_aborted.numel() > 0:
            metrics.update(
                {
                    "length_penalty/mean": lp_last_non_aborted.mean().detach().item(),
                    "length_penalty/max": lp_last_non_aborted.max().detach().item(),
                    "length_penalty/min": lp_last_non_aborted.min().detach().item(),
                    "length_penalty/nonzero_frac_samples": (lp_last_non_aborted > 0).float().mean().detach().item(),
                }
            )

            # Fraction of groups (by uid) with any non-zero penalty
            try:
                if "uid" in batch.non_tensor_batch:
                    uids = batch.non_tensor_batch["uid"]
                    # Align mask to numpy for indexing
                    nb_mask_np = non_aborted_mask.detach().cpu().numpy().astype(bool)
                    uids_nb = np.asarray(uids, dtype=object)[nb_mask_np]
                    lp_nb = lp_last_non_aborted.detach().cpu().numpy()

                    gid2has = {}
                    for gid, val in zip(uids_nb, lp_nb, strict=False):
                        key = str(gid)
                        if key not in gid2has:
                            gid2has[key] = False
                        if val > 0:
                            gid2has[key] = True

                    if gid2has:
                        frac = sum(1.0 for v in gid2has.values() if v) / float(len(gid2has))
                        metrics["length_penalty/nonzero_frac_groups"] = float(frac)
            except Exception:
                # Best-effort: skip group metrics if inputs are not aligned
                pass
...
def summarize_reward_extra(values_by_key: dict[str, Any], prefix: str) -> dict[str, float]:
    """Convert reward extra info into scalar metrics with a configurable prefix."""
    ...
    for key, raw_values in values_by_key.items():
        ...
        array = np.asarray(raw_values, dtype=object)
        if array.size == 0:
            continue
        ...
        for value in cleaned:
            if isinstance(value, (int, float, np.number)) and not isinstance(value, bool):
                ...
                categorical_vals.append(str(value))

        if numeric_vals:
            numeric_arr = np.asarray(numeric_vals, dtype=np.float64)
            if len(numeric_vals) == len(cleaned):
                metrics[f"{prefix}/{key}/mean"] = float(numeric_arr.mean())
                metrics[f"{prefix}/{key}/max"] = float(numeric_arr.max())
                metrics[f"{prefix}/{key}/min"] = float(numeric_arr.min())
            else:
                metrics[f"{prefix}/{key}/numeric_mean"] = float(numeric_arr.mean())
                metrics[f"{prefix}/{key}/numeric_max"] = float(numeric_arr.max())
                metrics[f"{prefix}/{key}/numeric_min"] = float(numeric_arr.min())

        if categorical_vals:
            counts = Counter(categorical_vals)
            total = float(len(categorical_vals))
...
def compute_reward_extra_metrics(batch: DataProto) -> dict[str, float]:
    """Aggregate reward extra info stored in the batch into scalar metrics."""

    meta_info = batch.meta_info or {}
    extra_keys = meta_info.get("reward_extra_keys")
    if not extra_keys:
        return {}

    values_by_key = {
        key: batch.non_tensor_batch.get(key)
        for key in extra_keys
        if batch.non_tensor_batch.get(key) is not None
    }

    return summarize_reward_extra(values_by_key, prefix="reward_extra")
