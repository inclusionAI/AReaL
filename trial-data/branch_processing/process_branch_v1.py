...
# (all special parallel-reasoning tokens except Path)
_SPECIAL_TAG_PATTERN = re.compile(
    r"<(Parallel|Goal|Trial|Subtask|Conclusion)>.*?</\1>",
    re.DOTALL,
)

# Matches a <Path>...</Path> block; used to extract content while stripping
# the opening tag, closing tag, and leading numbering (e.g. "1. " or "2. ").
_RE_PATH_BLOCK = re.compile(r"<Path>\s*\d+\.\s*", re.DOTALL)
...
def strip_special_tokens(text: str) -> str:
    ...
    (Parallel, Goal, Trial, Subtask, Conclusion) **and** their tags,
    as well as the "Let's think in parallel" trigger phrase.

    <Path> blocks are treated differently: the tags and the leading
    numbering (e.g. "1. " or "2. ") are removed but the path content
    itself is **preserved** in the output.

    The result is the plain reasoning text without any scaffolding.
    """
    # Step 1: Strip <Path> tags and leading numbers but keep the content.
    # Replace "<Path>\nN. " with nothing (keep what follows until </Path>),
    # and strip the closing </Path> tag.
    cleaned = _RE_PATH_BLOCK.sub("", text)
    cleaned = cleaned.replace("</Path>", "")

    # Step 2: Iteratively strip the remaining special tags (and their content)
    # until none remain. This handles nesting correctly.
    prev = None
    while prev != cleaned:
        prev = cleaned
        cleaned = _SPECIAL_TAG_PATTERN.sub("", cleaned)
