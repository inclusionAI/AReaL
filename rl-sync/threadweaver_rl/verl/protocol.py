...
def pad_dataproto_to_divisor(data: "DataProto", size_divisor: int):
    ...
    if len(data) % size_divisor != 0:
        ...
        data_padded.non_tensor_batch = dict(data_padded.non_tensor_batch)

        total_size = len(data_padded)
        pad_start = total_size - pad_size

        if "uid" in data_padded.non_tensor_batch:
            uid_array = data_padded.non_tensor_batch["uid"].copy()
            assert isinstance(uid_array, np.ndarray)
            for idx in range(pad_start, total_size):
                base_uid = uid_array[idx]
                uid_array[idx] = f"{base_uid}__pad__{uuid.uuid4()}"
            data_padded.non_tensor_batch["uid"] = uid_array

        padding_flag = data_padded.non_tensor_batch.get("__is_padding__")
        if padding_flag is None:
            padding_flag = np.zeros(total_size, dtype=bool)
        else:
            padding_flag = padding_flag.copy()
        padding_flag[pad_start:] = True
        data_padded.non_tensor_batch["__is_padding__"] = padding_flag
    else:
        if len(data) == 0:
    ...
    for key, val in tensor_dict2.items():
        if key in tensor_dict1:
            ...
            assert _deep_equal(tensor_dict1[key], tensor_dict2[key], visited=set()), (
                f"`{key}` in tensor_dict1 and tensor_dict2 are not the same object. {tensor_dict1[key]=}, {tensor_dict2[key]=}"
            )
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        for key, val in non_tensor_batch.items():
            try:
                ...
                shapes = [getattr(item, "shape", None) for item in val]
                raise ValueError(
                    "Failed to concatenate non-tensor field "
                    f"'{key}' with shapes {shapes}"
                ) from exc
