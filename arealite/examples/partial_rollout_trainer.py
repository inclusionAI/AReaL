...
def main(args):
    ...
    if config.partial_rollout.mini_samples_per_group != group_size:
        logger.warning(f"config.partial_rollout.mini_samples_per_group {config.partial_rollout.mini_samples_per_group} must be equal to group_size {group_size}")
    ...
    def add_res_to_rollout_buffer_callback(res: List[TensorDict]):
        ...
        for r in res:
            ...
            logger.info(f"add sample q[{r['query_id'][0]}]i[{r['index_in_group'][0]}] into rollout buffer, \
    ...
    for epoch in range(recover_epoch, epoch_num):
        ...
        for step in range(start_step, step_num):
            ...
            with (
                ...
                lack_samples = training_real_batch_size + config.partial_rollout.batch_size_exceeding_num - len(batch_data)
                ...
                logger.info(f"add {(lack_samples + group_size  - 1) // group_size} samples to batch_data, batch_data len is {len(batch_data)}, step: {step}, epoch: {epoch}, global_step: {global_step}")
