...
class MockAsystemAPIServer(BaseHTTPRequestHandler):
    ...
    def _generate_worker_statuses(self, job_request: Dict[str, Any]) -> List[List[Dict[str, Any]]]:
        """根据作业请求生成WorkerGroupStatuses"""
        worker_group_statuses = []
        
        for group_idx, worker_group in enumerate(job_request.get("asystemWorkerGroups", [])):
            ...
            for replica_idx in range(replicas):
                ...
                for container_idx, container_spec in enumerate(container_specs):
                    ...
                    # 生成端口列表
                    base_port = 8080 + group_idx * 100 + replica_idx * 10 + container_idx
                    ports = [base_port + i for i in range(port_count)]
                    
                    container_status = {
                        "name": container_spec.get("name", f"container_{container_idx}"),
                        "ports": ports
                    }
                    worker_status["containerStatuses"].append(container_status)
                
                # 处理worker组级别的portCount
                if not worker_status["containerStatuses"] and "portCount" in worker_group:
...
class MockWorkerRPCServer(BaseHTTPRequestHandler):
    ...
    def _handle_create_engine(self, data):
        ...
        try:
            ...
            # 获取worker标识
            worker_id = f"{self.server.server_address[0]}:{self.server.server_address[1]}"
    ...
    def _handle_call(self, data):
        ...
        try:
            ...
            logging.info(f"MockWorker: Called {method} on {worker_id}, result: {result}")
...
class TestRealAsystemAPI:
    ...
    def setup_mock_servers(self):
        """启动mock服务器"""
        # 启动Asystem API服务器
        self.asystem_server = HTTPServer(("127.0.0.1", self.asystem_port), MockAsystemAPIServer)
        asystem_thread = threading.Thread(target=self.asystem_server.serve_forever, daemon=True)
    ...
    def test_full_workflow(self):
        ...
        try:
            ...
            for i, worker in enumerate(workers):
                ...
                print(f"  - Engine creation on {worker.id}: {'✅ SUCCESS' if result else '❌ FAILED'}")
            ...
            print(f"Engine creation success rate: {success_count}/{len(engine_results)}")
