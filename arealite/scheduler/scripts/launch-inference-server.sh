...
WORKER_LOG_FILE=${WORKER_LOG_DIR}/${WORKER_TYPE}-sglang-${WORKER_INDEX}.log
...
# 定义同步使用的文件和路径
LOCK_FILE="/tmp/copy_sglang.lock"
DONE_FILE="/tmp/copy_sglang_done.flag"
...
sync_copy() {
    ...
    if flock -n 9; then
        ...
        if [[ ! -f "$DONE_FILE" ]]; then
            # 执行复制操作 (替换为您的实际命令)
            if cp ${WORKER_IMAGE} /tmp/image_sglang.sif; then
                touch "$DONE_FILE"
...
singularity --debug exec --pid --nv --no-home --writable-tmpfs --bind /storage:/storage "${WORKER_IMAGE}" bash -c "export PATH=/opt/conda/bin:$PATH; export PYTHONPATH=${REAL_PACKAGE_PATH}:$PYTHONPATH;python -u inference_server.py" >> ${WORKER_LOG_FILE} 2>&1
