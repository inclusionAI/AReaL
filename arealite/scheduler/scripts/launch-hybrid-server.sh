...
singularity --debug exec --pid --nv --no-home --writable-tmpfs --bind /storage:/storage "${WORKER_IMAGE}" bash -c "export PATH=/opt/conda/bin:$PATH; export PYTHONPATH=${ENGINE_PACKAGE_PATH}:/storage/openpsi/codes/antllm:/storage/openpsi/codes/atorch:/storage/openpsi/codes/Megatron-LM:/storage/openpsi/codes/Megatron-LM/ant_utils/dcp_utils:/root/workdir/astra-build/Asystem-HybridEngine/astra_cache/astra-client/python:$PYTHONPATH;$CMD" >> ${WORKER_LOG_FILE} 2>&1
