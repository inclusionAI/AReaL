...
class MockSchedulerAPIServer(BaseHTTPRequestHandler):
    ...
    def _handle_asystem_jobs(self):
        ...
        try:
            ...
            logger.info(
                f"请求内容: {json.dumps(job_request, indent=2, ensure_ascii=False)}"
            )
            ...
            self.end_headers()
            self.wfile.write(
                json.dumps({"error": "Invalid JSON format", "details": str(e)}).encode()
            )
        except Exception as e:
        ...
        if "containerSpecList" in group:
            ...
            if not isinstance(container_specs, list) or len(container_specs) == 0:
                ...
                for j, spec in enumerate(container_specs):
                    spec_errors = self._validate_container_spec(
                        spec, f"{prefix}.containerSpecList[{j}]"
                    )
                    ...
                    "containerSpecList": [
                        {
                            ...
                            "mounts": ["/mnt/host_data:/data"],
                            "resourceRequirement": {"cpu": 1000, "memoryMB": 2048},
                        }
                    ],
                },
            ],
