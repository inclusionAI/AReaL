...
def build_app(
    ...
    @app.post("/v1/{session_id}/chat/completions", dependencies=[Depends(validate_json_request)])
    async def chat_completions(request: AReaLChatCompletionRequest, session_id: str) -> ChatCompletion:
        state = get_shared_data()
