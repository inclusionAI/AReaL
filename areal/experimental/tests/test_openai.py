...
async def test_multi_round_conversation(openai_client):
    ...
    c1 = await openai_client.chat.completions.create(messages=messages, max_completion_tokens=8192)
    ...
    c2 = await openai_client.chat.completions.create(messages=messages ,max_completion_tokens=8192)
...
@pytest.mark.asyncio
async def test_multi_round_conversation_concat_style_export_with_stop_token_ids(openai_client, tokenizer):
    """Create a conversation tree using create() and verify parents and rewards.
