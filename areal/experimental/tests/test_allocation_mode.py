...
from areal.api.alloc_mode import AllocationValidationError
...
def test_colocate():
    ...
    assert alloc_mode.type_ == AllocationType.COLOCATE
    assert (
        alloc_mode.train_backend is None
    )  # auto set train backend should be None for hybrid without explicit backend
    train_ps = alloc_mode.train
    assert ParallelStrategy.parallelism_eq(
        train_ps,
        ParallelStrategy(
    ...
    alloc_mode = AllocationMode.from_str(alloc_mode_str)
    assert alloc_mode.type_ == AllocationType.COLOCATE
    assert alloc_mode.train_backend is None
    train_ps = alloc_mode.train
    assert ParallelStrategy.parallelism_eq(
        train_ps,
        ParallelStrategy(
