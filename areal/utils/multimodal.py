...
class VisionCollator:
    ...
    def __call__(self, data: List[Dict[str, Any]]) -> TensorDict:
        prompts = [message for message in data["messages"]]
        conversations = [conversation for conversation in data["conversation"]]
        ...
        if "videos" in data:
            ...
            # input_ids = self.processor.tokenizer(prompts, padding=False, return_tensors="pt")['input_ids'].tolist()[0]
            ...
            processed_input["images"] = image2base64(data["images"]) 
        else:
