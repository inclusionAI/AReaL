...
def test_tree_training_forward(engine, mock_tree_input):
    ...
    if not is_close.all():
        ...
        logger.error(f"Assertion failed: {num_mismatched}/{total_elements} elements mismatched ({mismatch_percentage:.2f}%)")
        ...
        for i in range(num_to_show):
            ...
            logger.error(f"  Position {idx}: tree={tree_val:.6f}, baseline={baseline_val:.6f}, "
                        f"abs_diff={abs_diff:.6f}, rel_diff={rel_diff:.6f}")
        
        # Summary statistics
        abs_diff_all = (logprob_tree - logprob_baseline).abs()
        logger.error(f"Overall abs diff: max={abs_diff_all.max().item():.6f}, "
...
def test_tree_training_forward_backward(mock_tree_input):
    ...
    for name in sorted(common_keys):
        ...
        if mean_diff > 1e-3:
            ...
            logger.info(f"Gradient mismatch for {name}:"
