...
def _patch_model_runner() -> None:
    ...
    def _patched_init_weights_update_group(
        ...
        if pp_rank is not None and self.pp_rank != pp_rank:
            ...
            logger.info(
                "Skipping init_weights_update_group for group '%s': "
                "requested pp_rank=%d but this worker is pp_rank=%d "
                "(tp_rank=%d).",
                group_name,
                pp_rank,
                self.pp_rank,
    ...
    def _patched_update_weights_from_distributed(
        ...
        if pg is None:
            ...
            logger.debug(
                ...
                "this worker (pp_rank=%d, tp_rank=%d) did not join the group.",
                group_name,
                self.pp_rank,
                self.tp_rank,
            )
    ...
    def _patched_destroy_weights_update_group(
        ...
        if pg is None:
            ...
            logger.debug(
                "Skipping destroy_weights_update_group for group '%s': "
                "this worker (pp_rank=%d, tp_rank=%d) did not join the group.",
                group_name,
                self.pp_rank,
                self.tp_rank,
