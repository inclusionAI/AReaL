...
class TrainEngine(abc.ABC):
    ...
    def forward_backward_batch(
        ...
        ----------
        mb_list : MicroBatchList
            The micro-batch list, which is iterable and yields (mbs, padded_mbs, padding_length, old_cu_seqlens).
        process_output : Callable[[torch.Tensor, dict[str, Any]], tuple[torch.Tensor | None, Any]]
            A function that processes the model output (logits) and returns a tuple:

            - loss (torch.Tensor | None): If not None, backward() will be called on this tensor.
            - result (Any): Arbitrary result for the caller to collect. Can be None if not needed.

            The function signature is: ``(logits: Tensor, inputs: dict) -> (loss, result)``
