...
def main():
    ...
    try:
        ...
        success = scheduler.create_engine(
            first_worker.id, engine, {"init_param": "test"}
        )
        if success:
        ...
        result = scheduler.call_engine(
            first_worker.id, "agenerate", req={"text": "What is machine learning?"}
        )
        print(f"推理结果: {result}")
...
def test_compatibility_with_local_scheduler():
    ...
    # 可以用同样的方式创建调度器
    AsystemScheduler(asystem_config)
