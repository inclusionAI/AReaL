...
类似于 areal/scheduler/test/test_local.py 的测试方式
"""

import logging

from areal.scheduler.asystem import AsystemScheduler
from areal.scheduler.base import ContainerSpec, SchedulingConfig
...
def main():
    ...
    # 创建调度配置
    scheduling_config = SchedulingConfig(replicas=2, specs=[spec])
    ...
    try:
        ...
        logging.info(
            f"Using worker with id={worker_id}, ip={worker.ip}, ports={worker.ports}"
        )
