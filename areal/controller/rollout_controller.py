...
from areal.api.cli_args import RolloutControllerConfig
from areal.api.controller_api import RolloutController
...
from areal.extension.asystem.remote_sglang_engine import (
    RemoteSGLangEngine,
...
class DistributedRolloutController(RolloutController):
    ...
    def initialize(self, *args, **kwargs):
        ...
        # rollout_controller中不需要单独指定这个环境变量 因为这个镜像中只有一个nccl @卮言
        ...
        if method == "wait_at_least_no_concat":
            ...
            for index in range(self.allocate_mode.gen_dp_size):
                ...
                self._start_wait_at_least_no_concat_loop(
                    ...
                    no_response_timeout=kwargs.get("no_response_timeout", None),
                )
        else:
