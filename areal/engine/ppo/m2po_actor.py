...
class M2POActor:
    """M2PO actor implementation built on top of PPOActor ideas.

    Key differences:
    - adds a margin-based group loss that encourages group-wise margin between
      top-performing samples and others
    - reuses existing PPO loss components for policy update
    """
    ...
    def __init__(self, config: PPOActorConfig, engine: TrainEngine):
        ...
        # reuse reward scaling/bias and group config
        ...
        # M2PO specific
        self.margin = getattr(config, "m2po_margin", 0.1)
        self.margin_coef = getattr(config, "m2po_margin_coef", 1.0)
    ...
    def compute_advantages(self, data: Dict[str, Any]) -> None:
        # reuse the same logic as PPOActor.compute_advantages
    ...
    def ppo_update(self, data: Dict[str, Any]) -> List[Dict[str, float]]:
        # dynamic sampling as in PPOActor
...
class FSDPM2POActor(FSDPEngine):
    def __init__(self, config: PPOActorConfig):
        super().__init__(config)
        self.actor = M2POActor(config, self)
...
def m2po_loss_fn(
    ...
    """M2PO loss wraps PPO loss and adds a margin ranking loss across a group.

    margin loss: for each group, encourage the top-performing sample's
    advantage to be greater than others by at least `margin`.
    """
    ...
    # compute margin ranking loss across groups
    if group_size is not None and group_size > 1:
        # assume batch is arranged so that consecutive `group_size` sequences form a group
        B = advantages.shape[0]
        g = group_size
        num_groups = B // g
        adv_pairs = advantages.view(num_groups, g, -1)
        # reduce each sequence to a scalar score by summing valid tokens
        seq_scores = (adv_pairs.sum(dim=-1) * (loss_mask.view(num_groups, g, -1).sum(-1) > 0).float()).float()
        # pick top score per group
        top_scores, _ = seq_scores.max(dim=1, keepdim=True)
        # margin violation
        margin_violations = torch.relu(margin - (top_scores - seq_scores))
        margin_loss = margin_violations.mean()
        loss = loss + margin_coef * margin_loss
        stats_tracker.scalar(m2po_margin_loss=margin_loss.item())

    # Logging similar statistics as grpo_loss_fn
