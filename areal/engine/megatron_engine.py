...
class MegatronEngine(BaseTrainEngine):
    """Megatron-Core based training engine with 3D parallelism support.

    Implements :class:`BaseTrainEngine` using Megatron-Core for distributed
    training with support for data parallelism (DP), tensor parallelism (TP),
    pipeline parallelism (PP), context parallelism (CP), and virtual pipeline
    parallelism (VPP).

    The micro-batch input for :meth:`forward_compute_mb` is a :class:`MicroBatchItem`
    namedtuple containing ``orig_mb``, ``padded_mb``, ``padding_length``, and
    ``old_cu_seqlens``.
    """
    ...
    def backend_requires_loss_tensor(self) -> bool:
        """Megatron schedule requires loss to be a tensor even when forward_only=True."""
        return True
    ...
    def forward_compute_mb(
        ...
        **kwargs,
        ...
        def _process_output_fn(input_, output_):
            return process_output_fn(output_, input_)
        ...
        return output, functools.partial(_process_output_fn, mb_input.orig_mb)
