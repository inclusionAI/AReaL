...
class MegatronEngine(TrainEngine):
    ...
    def _init_weight_update_from_distributed(self, meta: WeightUpdateMeta) -> None:
        ...
        For PP=1 the behaviour is identical to the original: a single NCCL
        group spanning the entire inference fleet + one training source rank.
        ...
        os.environ["TORCHELASTIC_USE_AGENT_STORE"] = str(False)  # FIX: 恢复
        ...
        self.engine_lock.acquire()  # FIX: 恢复锁

        try:
            if pp_size > 1:
                # ---- Per-PP-rank NCCL group (NEW) ----------------------------
                pp_rank = mpu.get_pipeline_model_parallel_rank()
                ...
                self.logger.info(
                    f"Initializing per-PP weight update group: type={meta.type} "
                    f"init_method={init_method} "
                    f"group=update_weight_group_{pp_rank} "
                    f"world_size={per_pp_world_size}"
                )
                ...
                # ---- Original single-group mode (PP=1) ----------------------
                # FIX: 恢复整个 PP=1 分支！这是 CRITICAL BUG 的修复
                fut = self.rollout_engine.init_weights_update_group(meta)

                gen_world_size = gen_parallel.world_size
                init_method = f"tcp://{format_host_for_url(meta.nccl_master_address)}:{meta.nccl_master_port}"
                self.logger.info(
    ...
    def _update_weights_from_distributed(self, meta: WeightUpdateMeta) -> None:
        ...
        # Set pp_rank for per-PP-rank mode
        gen_parallel = meta.gen_allocation.parallel
        if gen_parallel.pp_size > 1 and self.is_pipeline_parallel_head():
