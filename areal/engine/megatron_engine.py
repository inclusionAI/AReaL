...
class MegatronEngine(TrainEngine):
    ...
    def _configure_pipeline_layer_splits(self) -> None:
        if self.parallel_strategy is None:
            return
        pp_size = getattr(self.parallel_strategy, "pipeline_parallel_size", 1)
        if pp_size <= 1:
            return

        total_layers = getattr(self.tf_config, "num_layers", None)
        if not isinstance(total_layers, int) or total_layers <= 0:
            return

        layer_param_weights, embedding_params, output_params = (
            self._estimate_stage_parameter_buckets()
        )
        if not layer_param_weights or all(
            weight <= 0 for weight in layer_param_weights
        ):
            return
        ...
        def span_sum(start: int, length: int) -> float:
            if length <= 0:
                return 0.0
            end = min(start + length, total_layers)
            if end <= start:
                return 0.0
        ...
        if pp_size == 2:
            for first_layers in range(total_layers + 1):
                last_layers = total_layers - first_layers
                first_load = embedding_params + span_sum(0, first_layers)
                last_start = total_layers - last_layers
                last_load = output_params + span_sum(last_start, last_layers)
                max_load = max(first_load, last_load)
                diff = abs(first_load - last_load)
                if diff < best_diff or (diff == best_diff and max_load < best_max):
                    best_diff = diff
                    best_max = max_load
                    best_choice = (first_layers, last_layers, 0)
        else:
            middle_stage_count = pp_size - 2
            for first_layers in range(total_layers + 1):
                remaining_after_first = total_layers - first_layers
                if remaining_after_first < 0:
                    break
                for last_layers in range(remaining_after_first + 1):
                    remaining_layers = total_layers - first_layers - last_layers
                    if remaining_layers < 0:
                        continue
                    if (
                        middle_stage_count > 0
                        and remaining_layers % middle_stage_count != 0
                    ):
                        continue
                    middle_layers = (
                        remaining_layers // middle_stage_count
                        if middle_stage_count
                        else 0
                    )
                    first_load = embedding_params + span_sum(0, first_layers)
                    loads: list[float] = [first_load]
                    cursor = first_layers
                    for _ in range(middle_stage_count):
                        loads.append(span_sum(cursor, middle_layers))
                        cursor += middle_layers
                    last_start = total_layers - last_layers
                    last_load = output_params + span_sum(last_start, last_layers)
                    loads.append(last_load)
                    max_load = max(loads)
                    min_load = min(loads)
                    ...
                    if diff < best_diff or (diff == best_diff and max_load < best_max):
                        best_diff = diff
                        best_max = max_load
                        best_choice = (first_layers, last_layers, middle_layers)

        if best_choice is None:
            return

        first_layers, last_layers, middle_layers = best_choice
        setattr(self.tf_config, "num_layers_in_first_pipeline_stage", int(first_layers))
        setattr(self.tf_config, "num_layers_in_last_pipeline_stage", int(last_layers))
        if pp_size > 2 and hasattr(
            self.tf_config, "num_layers_in_middle_pipeline_stage"
        ):
            setattr(
                self.tf_config,
                "num_layers_in_middle_pipeline_stage",
                int(middle_layers),
            )

        if hasattr(logger, "debug"):
            logger.debug(
                "Pipeline layer split determined: first=%s, middle=%s, last=%s (pp=%s)",
                first_layers,
                middle_layers if pp_size > 2 else None,
                last_layers,
                pp_size,
            )

    def _estimate_stage_parameter_buckets(self) -> tuple[list[float], float, float]:
        hf_conf = self.hf_config
        tf_conf = self.tf_config
        ...
        if num_moe_experts:
            ...
            expert_parallel_size = (
                getattr(tf_conf, "expert_model_parallel_size", 1) or 1
            )
            num_local_experts = math.ceil(
                num_moe_experts / max(expert_parallel_size, 1)
            )
