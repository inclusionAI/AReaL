...
def compute_rw_loss(rewards: torch.Tensor, input_: TensorDict) -> torch.Tensor:
    ...
    for i in range(bs):
        ...
        c_inds = torch.cat([(c_attention_mask == 0).nonzero(),
                                torch.tensor([[len(chosen_id)]]).to(c_attention_mask.device)])
        r_inds = torch.cat([(r_attention_mask == 0).nonzero(),
        ...
        acc = 1. if chosen_score > rejected_score else 0.
        ...
        stats_tracker.denominator(
            n_seqs=torch.ones(
                1, dtype=torch.bool, device=loss.device
            ),
        )
