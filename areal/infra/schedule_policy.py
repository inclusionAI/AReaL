        ...
        while chosen_worker is None:
            async with self._lock:
                ...
                if (
                    ...
                    logger.info(
                        f"{asyncio.current_task().get_name()} chooses worker: {highest_priority_worker.id}"
                    )
            ...
            if chosen_worker is None:
                logger.info(
                    f"{asyncio.current_task().get_name()} is waiting to choose worker..."
                )
                await self._idle_event.wait()
                self._idle_event.clear()
                logger.info(
                    f"{asyncio.current_task().get_name()} is notified to choose worker..."
                )
        return chosen_worker
        ...
        async with self._lock:
            for i, (process_requests_count, (worker_id, _)) in enumerate(
                ...
                if worker.id == worker_id:
                    ...
                    logger.info(
                        f"{asyncio.current_task().get_name()} has released worker {worker.id=} ..."
                    )
                    return
            raise RuntimeError(
                f"No workers available to release.{worker_id=} {worker.id=}"
            )
...
class RoundRobinSchedulePolicy(SchedulePolicy):
    ...
    async def choose_worker(self):
        if self.no_block:
            return self._choose_worker_no_block()
    ...
    def _choose_worker_no_block(self):
