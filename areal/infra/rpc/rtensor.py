            ...
            for k, v in obj.items():
                if is_multi_modal_key(k):
                    if group_indices is not None and isinstance(v, list):
                        # Expand v if it has fewer entries than max index
                        # (happens when n_samples > 1 and multi_modal_input
                        # was not fully replicated during RPC transport)
                        max_idx = max(max(g) for g in group_indices) + 1
                        if len(v) < max_idx and max_idx % len(v) == 0:
                            n_rep = max_idx // len(v)
                            expanded = []
                            for entry in v:
                                expanded.extend([entry] * n_rep)
                            v = expanded
                        split_values[k] = [
