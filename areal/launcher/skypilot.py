...
class SkyPilotLauncher:
    def __init__(
        ...
        self.config = config
        self.skypilot_config = config.skypilot
        self.cluster_spec_config = cluster_spec_config

        self.cluster_name = _readable_cluster_name(experiment_name, trial_name)
        self._ensure_cluster()

    def _build_resources(self) -> sky.Resources:
        assert self.skypilot_config.accelerator_type is not None
        self.cluster_spec_config.n_nodes
        accelerator_type = self.skypilot_config.accelerator_type
        n_gpus_per_node = self.cluster_spec_config.n_gpus_per_node
        accelerators = f"{accelerator_type}:{n_gpus_per_node}"
        # TODO: build sky.Resources according to SkyPilotLauncherConfig cli_args.py
    ...
    def _base_task(
        ...
        base_envs = _parse_key_value_pairs(self.skypilot_config.envs)
        secrets = _parse_key_value_pairs(self.skypilot_config.secrets)
        if secrets:
            base_envs.update(secrets)
        workdir = _default_workdir(self.skypilot_config)
        file_mounts = None
        if self.skypilot_config.file_mounts:
            file_mounts = _parse_yaml_like(self.skypilot_config.file_mounts)
        resources = self._build_resources(self.skypilot_config)
