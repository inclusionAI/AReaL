...
{objective}
    ...
    async def arun_episode(self, engine: InferenceEngine, data):
        ...
        # Clip each tensor dict to 16384 tokens if it exceeds this limit
            ...
            if seq_length > max_length:
                ...
                for key, tensor in tensor_dict.items():
                    if key in ['input_ids', 'logprobs', 'loss_mask', 'versions', 'attention_mask']:
                        # Clip the sequence dimension (assuming it's the last dimension)
                        clipped_dict[key] = tensor[:, :max_length]
                    else:
                        # Keep other fields unchanged (like rewards)
                        clipped_dict[key] = tensor
                
                # Create new TensorDict with clipped tensors
                ...
                # Keep original tensor dict if it's within the limit
