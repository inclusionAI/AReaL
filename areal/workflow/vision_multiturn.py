...
class VisionMultiTurnWorkflow(RolloutWorkflow):
    ...
    def __init__(
        ...
        failure_feedback_msg = "Your answer is either wrong or not parsable to the reward function. Try to answer it again. The final answer MUST BE put in \\boxed{}."
        self.feedback_str = [
            {
                "role": "user",
                "content": [{"type": "text", "text": failure_feedback_msg}],
            }
        ]
        messages += self.feedback_str
        s2 = list(
            self.tokenizer.apply_chat_template(
                messages, tokenize=True, add_generation_prompt=True
            )
        )
        self.feedback_ids = s2[len(s1) :]
    ...
    async def arun_episode(
        ...
        for turn in range(self.max_turns):
            ...
            if (
                ...
                new_tokens.append(self.tokenizer.eos_token_id)
                logprobs.append(0.0)
                loss_mask.append(0)
                versions.append(-1)
            ...
            seq.extend(self.feedback_ids)

            if messages_chat:
                messages_chat = messages_chat + self.feedback_str

            logprobs.extend([0.0] * len(self.feedback_ids))
            loss_mask.extend([0] * len(self.feedback_ids))
            versions.extend([-1] * len(self.feedback_ids))
