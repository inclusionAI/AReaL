...
def extract_thread_result(thread_response: str, thread_id: str) -> str:
    ...
    if result_start_match:
        ...
        return thread_response[start_pos:] + f"</thread_result>"
    else:
...
class ParallelThinkingWorkflow(RolloutWorkflow):
    ...
    def __init__(
        ...
        max_context_length: int = 10000,  # Maximum context length before stopping
    ):
    ...
    async def _run_one_episode(self, engine: InferenceEngine, data, rid):
        ...
        while reward == 0 and t < self.max_turns:
          ...
          #print("Sleep 5 s")
            ...
            if not main_response.endswith("</launch_threads>") and "<launch_threads>" in main_response:
            ...
            while True:
                ...
                thread_results, thread_seq_data_list = await self._process_threads_parallel(engine, problem_text, reasoning_context, thread_infos, rid, t, thread_turn_count, all_generations, all_generated_content)
      ...
      #print(f"Sleep 5 s")
        ...
        return (
            ...
            reward,
            len(seq),
            all_generations,  # Add all_generations to return
        )
      ...
      #print("[DEBUG] enter arun_episode", flush=True)
        ...
        if self.dump_dir is not None:
            ...
            async with aiofiles.open(file_path, "a") as f:
                ...
                for i, (_, p, c, r, sl, all_gens) in enumerate(results):
                    info = "\n".join(
                        [
        ...
        for result in results:
            result_list, _, _, _, _, _ = result  # Unpack to get the list of TensorDicts
            all_tensor_dicts.extend(result_list)  # Add all TensorDicts from this result
        
        return concat_padded_tensors(all_tensor_dicts)