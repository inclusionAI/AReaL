...
from areal.api.io_struct import LLMResponse
from areal.workflow.partial_rollout import PartialRolloutWorkflow
...
async def test_arun_episode_new_prompt(dummy_reward_fn, dummy_gconfig, dummy_tokenizer):
    workflow = PartialRolloutWorkflow(
        reward_fn=dummy_reward_fn, gconfig=dummy_gconfig, tokenizer=dummy_tokenizer
    )
...
@pytest.mark.asyncio
async def test_arun_episode_previous_ids(
    dummy_reward_fn, dummy_gconfig, dummy_tokenizer
):
    workflow = PartialRolloutWorkflow(
        reward_fn=dummy_reward_fn, gconfig=dummy_gconfig, tokenizer=dummy_tokenizer
    )
...
@pytest.mark.asyncio
async def test_arun_episode_variable_length_outputs(
    dummy_reward_fn, dummy_gconfig, dummy_tokenizer
):
...
@pytest.mark.asyncio
async def test_arun_episode_different_tasks(
    dummy_reward_fn, dummy_gconfig, dummy_tokenizer
):
...
@pytest.mark.asyncio
async def test_arun_episode_stop_reasons(
    dummy_reward_fn, dummy_gconfig, dummy_tokenizer
):
...
@pytest.mark.asyncio
async def test_arun_episode_complex_scenarios(
    dummy_reward_fn, dummy_gconfig, dummy_tokenizer
):
    ...
    long_prompt = (
        "This is a much longer prompt that should result in more input tokens " * 3
    )
    ...
    first_part_ids = td["input_ids"][0][
        : prompt_length + 2
    ].tolist()  # Take first 2 generated tokens
    ...
    continuation_results = await workflow.arun_episode(
        engine2, continuation_data.copy()
    )
...
def test_init_tokenizer_required(dummy_reward_fn, dummy_gconfig):
    with pytest.raises(ValueError):
        PartialRolloutWorkflow(reward_fn=dummy_reward_fn, gconfig=dummy_gconfig)
