...
async def general_verify(id2info, responses: list[str], query_ids: list) -> list[float]:
    ...
    try:
        ...
        if "error" in api_responses:
            token_lens = [
                len(reward_model_tokenizer.encode(txt, add_special_tokens=False))
                for txt in convs_formatted
            ]

            logger.error(
                f"General reward call failed with err msg: {api_responses}\n"
                f"Batch size: {len(convs_formatted)}, "
                f"each text token length: {token_lens}, "
                f"max single length: {max(token_lens)}, "
                f"total tokens: {sum(token_lens)}"
            )
