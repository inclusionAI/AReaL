...
class RemoteHybridTrainWorker(TrainEngine):
    def __init__(self, config: TrainEngineConfig):
        self.train_engine_config = config
        self.config = config.hybrid_engine
    ...
    def save(self, meta: SaveLoadMeta):
        ...
        try:
            ...
            if self.global_rank == 0:
                # megatron 侧 mkdir meta.path/iter_0000{global_step} 的时候出现 NotADirectoryError, 所以新增检查目录是否存在的逻辑
                if not os.path.exists(meta.path):
                    logger.warning(f"Directory {meta.path} does not exist, creating...")
                    os.makedirs(meta.path, exist_ok=True)
                    logger.info(
                        f"Directory {meta.path} has been recreated as empty directory"
                    )

                is_existed = os.path.exists(meta.path)
                is_empty_dir = os.path.isdir(meta.path) and not os.listdir(meta.path)
                logger.info(
                    f"save_dir: {meta.path} is existed: {is_existed}, is_empty_dir: {is_empty_dir}"
                )
    ...
    def train_batch(
        ...
        if "ref_logprobs" in batch_data and "seqlen" in batch_data:
            batch_data["packed_ref_logprobs"] = pack_ref_logprobs(
                batch_data["ref_logprobs"], batch_data["seqlen"]
            )
        ...
        for mb_i, data in enumerate(datas):
            train_stats = self.train_batch_helper(data, loss_fn, loss_weight_fn)
    ...
    def process_training_data(
        ...
        reward_score = torch.where(
            task_mask,
            (reward_score - self.config.wrap_policy.reward_output_bias)
            * self.config.wrap_policy.reward_output_scaling,
            reward_score,
        )
        ...
        if self.config.wrap_policy.recompute_logp:
            ...
            logprobs = self._compute_logprobs(compute_logp_input)
    ...
    def compute_logprobs(
        ...
        try:
            logprobs = self._compute_logprobs(batch)
            if logprobs is None:
                raise EngineError(
                    "RefEngineError",
                    "ComputeLogpError",
                    "Failed to exec compute_logprobs, logprobs is none",
                )
            logger.info(
                ...
                for batch_seqlens in seqlens:
                    for seqlen in batch_seqlens:
                        ...
                        padded = torch.nn.functional.pad(
                            seq_logprobs, (0, max_len - seqlen), value=0.0
                        )
