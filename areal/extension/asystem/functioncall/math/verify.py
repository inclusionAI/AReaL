...
async def math_verify(
    ...
    for batch_idx, results in enumerate(results_batch):
        ...
        if not (
            ...
            logger.warning(
                f"Invalid functioncall math results: {results}, math_provider={math_provider}, batch index:{batch_idx}, query index: {query_indices[index - 1]}, params: {batch_args_list[batch_idx]}."
            )
            continue
