"""Rollout-only script for Tau2 benchmark using GatewayRolloutController.
...
class Tau2GatewayRolloutConfig(BaseExperimentConfig):
    """Configuration for Tau2 rollout-only with GatewayRolloutController."""
...
def main(argv: list[str]) -> None:
    ...
    ctrl = GatewayRolloutController(config=ctrl_config, scheduler=scheduler)
    ctrl.initialize(
        role="rollout",
