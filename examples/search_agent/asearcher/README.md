...
```
sudo python3 -m areal.launcher.slurm ../ASearcher/train/asearcher_reasoning.py --config ../ASearcher/configs/asearcher_web_qwq.yaml experiment_name=gjx-asearcher-qwen72b-inst-server-only trial_name=run1 cluster.n_nodes=1 allocation_mode=sglang.d2t4p1 actor.path=/storage/openpsi/models/Qwen__Qwen2.5-72B-Instruct 
```
...
python3 -m areal.launcher.slurm \
    ...
    judge_engine.experiment_name=gjx-asearcher-qwen72b-inst-server-only \
