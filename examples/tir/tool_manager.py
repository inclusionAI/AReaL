...
class ToolRegistry:
    ...
    def __init__(self, timeout: int = 30, enabled_tools: str="python;calculator", debug_mode: bool = False):
        # All available tools
        self.all_tools = {
            ToolType.PYTHON: QwenPythonTool(timeout, debug_mode),
        ...
        self.tools = {tool_type: self.all_tools[tool_type] for tool_type in self.enabled_tools}
        
        logger.info(f"ToolRegistry initialized with enabled tools: {[t.value for t in self.enabled_tools]}")
...
class ToolManager:
    ...
    def __init__(self, timeout: int = 30, enabled_tools: str="python;calculator", debug_mode: bool = False):
        ...
        logger.info(f"Initialized ToolManager (debug_mode={debug_mode}, enabled_tools={[t.value for t in self.registry.get_enabled_tools()]})")
