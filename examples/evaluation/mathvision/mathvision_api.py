...
def pil_to_data_url(img: Image.Image) -> str:
    """将PIL图片转换为API需要的 data-url 格式 (image/jpeg)"""
    if img.mode != "RGB":
        img = img.convert("RGB")
    buf = io.BytesIO()
...
# --------------------------
# Math Extraction & Verification Logic
# --------------------------
...
def parse_number(s: str) -> Optional[Fraction]:
    ...
    if _NUM_SCI_RE.match(x):
        try:
            ...
            elif y.startswith("-"): sign = -1; y = y[1:]
            base, exp = re.split(r"[eE]", y)
            exp_i = int(exp)
            if "." in base: int_part, frac_part = base.split(".", 1)
            else: int_part, frac_part = base, ""
            digits = (int_part + frac_part).lstrip("0") or "0"
            frac_len = len(frac_part)
            pow10 = exp_i - frac_len
            if pow10 >= 0: return Fraction(sign * int(digits + ("0" * pow10)), 1)
            k = -pow10
            if len(digits) <= k: val = "0." + ("0" * (k - len(digits))) + digits
            else: val = digits[:-k] + "." + digits[-k:]
            if sign < 0: val = "-" + val
            return Fraction(val)
...
async def infer_one(
    ...
    """
    针对 matrixllm.alipay.com 优化的单次调用函数
    """
    # 1. 图片处理：转为 data:image/jpeg;base64,xxx
    data_url = pil_to_data_url(image)

    # 2. 构建 Prompt
    if len(options) > 0:
        assert len(options) == 5, f"Options length unexpected: {options}"
        # 兼容 MathVision 数据集格式
    ...
    # 4. 发送请求 (Header 在 session 中已统一配置)
    async with session.post(url, json=payload) as resp:
        ...
        try:
            ...
            raise RuntimeError(f"Unexpected response format: {out}") from e
...
async def run(args: argparse.Namespace) -> None:
    ...
    # 处理 URL：确保结尾没有多余斜杠，并拼接 chat/completions
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Eval script for MatrixLLM/OpenAI-compatible APIs")
    p.add_argument("--dataset", type=str, default="MathLLMs/MathVision")
    p.add_argument("--split", type=str, default="test")
    
    p.add_argument("--base-url", type=str, default="https://matrixllm.alipay.com/v1", help="API Base URL")
    p.add_argument("--api-key", type=str, required=True, help="API Key for the model service")
    p.add_argument("--model", type=str, required=True, help="Model name to call (e.g. claude-3-7-sonnet-20250219)")

    p.add_argument("--concurrency", type=int, default=32, help="Concurrent requests")
    p.add_argument("--timeout-s", type=int, default=3600, help="Request timeout in seconds")

    p.add_argument("--max-tokens", type=int, default=10240)
    p.add_argument("--temperature", type=float, default=0.0)
    
    p.add_argument("--tol", type=float, default=1e-6, help="Numerical tolerance for math check")
