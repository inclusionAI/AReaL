...
AReaL's trainer automatically provides `base_url`, `api_key`, and `http_client` during
RL training.
...
- Agent reads `base_url` from `os.environ["OPENAI_BASE_URL"]` and `api_key` from
  `os.environ["OPENAI_API_KEY"]` instead of `extra_kwargs`
...
All user-facing endpoints require authentication via `X-API-Key` header. The API key is
automatically generated by the `RolloutController` and passed to agents via
`extra_kwargs` (inline mode) or `OPENAI_API_KEY` environment variable (subprocess mode).
