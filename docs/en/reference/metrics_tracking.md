...
single-controller training, the controller may run on a CPU node while actor and
rollout workers run on GPU nodes. In that setup, ordinary `wandb.mode: "online"`
only reports controller process metrics.

To collect GPU utilization and memory from worker nodes, use W&B shared mode and
enable worker system metrics:
...
`x_update_finish_state=False`, so workers contribute only their system metrics and
do not finish the run.
