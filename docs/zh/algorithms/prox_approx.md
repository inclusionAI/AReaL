...
近似对数概率（Proximal log-probability approximation）是一种用于**解耦PPO**的优化技术，消除了计算近端策略对数概率以进行重要性比率计算所需的昂贵前向传播。
...
**`"loglinear"`（推荐）**
- 公式：$\log \pi_{prox} = \log \pi_{behave} + \alpha \cdot (\log \pi_{\theta} - \log \pi_{behave})$
...
**版本跟踪：**
每个生成的token都携带一个版本号，指示是哪个策略版本生成的它。近似使用这些版本来计算插值权重α。

**自动优化：**
当 `prox_logp_method="loglinear"` 时，前向传播在 `compute_logp()` 中自动跳过，用户脚本无需任何更改。
