...
请注意，内部后端配置不影响 AReaL 如何分配 GPU。给定分配模式 `sglang:d4t4`，你还可以配置 `sglang.dp_size=4`、`sglang.ep_size=4` 和 `sglang.enable_dp_attention=True`。在这种情况下，我们启动 4 个模型副本，每个副本 4 个 GPU。在每个实例内，SGLang 仍然使用 DP attention 和专家并行来分配注意力层和专家层中的计算。
...
这启用了 [MoE Parallel Folding](https://github.com/NVIDIA/Megatron-LM/tree/main/megatron/core/transformer/moe#moe-parallel-folding)，可以降低组合上下文和专家并行的最低 GPU 要求。
