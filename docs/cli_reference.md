...
| `load_device`               | string \| None                                      | `None`                | Device to load model weights during initialization. None uses the current device (GPU). 'cpu' loads weights to CPU, then broadcasts from rank 0 to all ranks after FSDP sharding. Use 'cpu' to prevent OOM for large models. Only effective with FSDP or Archon engine.                                                                                                                                                                                      |
