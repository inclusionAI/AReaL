...
class CustomRotaryPositionalEncoding(nn.Module):
	...
	def forward(self, x: torch.Tensor, position_ids: torch.LongTensor) -> Tuple[torch.Tensor, torch.Tensor]:
		...
		# Keep the first m dimensions as original
		cos = cos_original.clone()
		sin = sin_original.clone()
		
		# For the remaining (n-m) dimensions, create the special pattern
		if n > m:
			n_minus_m = n - m
			# Create indices for rows: 0, 1, 2, ..., (n-m-1)
			row_indices = torch.arange(n_minus_m, device=x.device, dtype=torch.float32)
			# Create indices for columns: 0, 1, 2, ..., (n-m-1)
			col_indices = torch.arange(n_minus_m, device=x.device, dtype=torch.float32)
			
			# Create the pattern: row_indices (outer) * col_indices (inner) * 2π / (n-m)
			# Shape: [n-m, n-m]
			row_indices_expanded = row_indices.unsqueeze(1)  # [n-m, 1]
			col_indices_expanded = col_indices.unsqueeze(0)  # [1, n-m]
			
			# Compute 2π * row * col / (n-m)
			angles = 2 * math.pi * row_indices_expanded * col_indices_expanded / n_minus_m
			
			# For cos (keys): use cosine of the angles
			# For sin (queries): use sine of the angles
			cos_pattern = torch.cos(angles)  # [n-m, n-m]
			sin_pattern = torch.sin(angles)  # [n-m, n-m]
			
			# Broadcast to batch and sequence dimensions and assign to the last (n-m) dimensions
			# We need to transpose because columns in the formula correspond to the embedding dimension
			# Expand to match [batch, seq_len, n-m] shape
			cos_pattern_expanded = cos_pattern.T.unsqueeze(0).unsqueeze(0).expand(batch, seq_len, -1).to(dtype=cos.dtype)
			sin_pattern_expanded = sin_pattern.T.unsqueeze(0).unsqueeze(0).expand(batch, seq_len, -1).to(dtype=sin.dtype)
			
			cos[:, :, m:n] = cos_pattern_expanded
			sin[:, :, m:n] = sin_pattern_expanded
