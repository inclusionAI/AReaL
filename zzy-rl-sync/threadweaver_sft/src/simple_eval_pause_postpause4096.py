...
def _continue_generation_from_prompt(
    prompt_text: str,
    stop_tokens_ids: List[int],
    continuation_token_quota: Union[int, None] = None,
    temperature: Union[float, None] = None,
    top_p: Union[float, None] = None,
) -> str:
    current_text = prompt_text
    remaining_quota = (
        None if continuation_token_quota is None else max(0, continuation_token_quota)
    )
    while True:
        ...
        max_tokens = get_post_pause_request_max_tokens(
            remaining_context_tokens, remaining_quota
        )
        if max_tokens <= 0:
            return current_text
        ...
        generated_chunk = completion.choices[0].text
        current_text += generated_chunk
        if remaining_quota is not None:
            consumed_tokens = len(
                tokenizer.encode(generated_chunk, add_special_tokens=False)
            )
            if consumed_tokens <= 0:
                consumed_tokens = max_tokens
            remaining_quota -= consumed_tokens
...
def _apply_hard_prethink_limit(
    ...
    if truncated and not suffix_from_close:
        ...
        full_text = _continue_generation_from_prompt(
            ...
            continuation_token_quota=POST_PAUSE_CONTINUATION_TOKEN_QUOTA,
        )
...
file_stem += f"_postpauseq{POST_PAUSE_CONTINUATION_TOKEN_QUOTA}"
...
metrics_payload = {
    ...
    "post_pause_continuation_token_quota": POST_PAUSE_CONTINUATION_TOKEN_QUOTA,
...
with open(report_md_file, "w") as f:
    ...
    f.write(
        f"- post_pause_continuation_token_quota: {POST_PAUSE_CONTINUATION_TOKEN_QUOTA}\n\n"
    )
    f.write("## Accuracy\n\n")
