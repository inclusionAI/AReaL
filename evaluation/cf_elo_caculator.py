...
def calc_elo_rating_offline(contest_id: int, problem_status: Dict[str, List[bool]], 
                           sorted_ratings: List[float], cached_data: Dict, pass_n=None) -> Optional[Tuple[int, float]]:
    try:
...
def convert_score_to_cf_format(all_samples: List[Dict], metadata: List[str]) -> List[List[bool]]:
    cf_results = []
...
class CFEloCalculator:
    def __init__(self, metadata_path: str = None, ratings_path: str = None, cache_file_path: str = None):
        # Set default paths
    ...
    def calculate_elo(self, all_samples: List[Dict], pass_n: int = 1, verbose: bool = True) -> Optional[Dict]:
        try:
            if verbose:
                print("\n" + "="*50)
            ...
            iterator = tqdm(model_results, desc="Calculating ELO ratings") if verbose else model_results
            ...
            if verbose:
                ...
                if skipped_contests:
                    print(f"Skipped contest IDs: {skipped_contests[:10]}{'...' if len(skipped_contests) > 10 else ''}")
...
def calculate_cf_elo_from_samples(all_samples: List[Dict], 
                                  pass_n: int = 1,
