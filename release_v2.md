## AReaL v1.0.0：统一的智能体强化学习训练，通用为本，原生致简

经过近一年的迭代打磨，AReaL 迎来了里程碑式的 **v1.0.0 稳定版本**。这不仅是一个版本号的跃迁，更标志着 AReaL 从"能用"走向"好用"的成熟蜕变。

AReaL 的愿景很简单：**让每个人都能轻松、低成本地训练自己的 AI Agent**。

## 在这个愿景的驱动下，我们构建了一套面向大规模推理与智能体模型的全异步强化学习训练系统。它不是又一个"能跑通 demo"的框架，而是一个经过万亿参数模型实战检验、支撑多个 SOTA 成果的生产级系统。
...
主流的强化学习框架大多将Agent实现耦合在框架中，采用批量训练模式：Agent生成轨迹，等待所有模型生成和工具调用完毕，然后统一训练，再生成下一批轨迹。这种交错生成模式的问题在agent场景下尤为突出——当你的推理集群有数百张卡时，一个长尾样本就能让整个系统空转等待。

**痛点二：Agent 框架接入成本高，缺乏统一标准**

将一个 Agent 框架接入 RL 训练却往往需要大量适配工作。LangChain、OpenAI Agents SDK、CAMEL-AI、claude code...
每个框架的接口都不一样，每接入一个新框架就要写一套新的胶水代码。更麻烦的是，Agent 框架通过高层 API 与 LLM 交互，无法直接获取 token 级别的信息，导致训推
token 不一致的隐患。
...
这种设计更让系统组件更模块化、功能化——例如，你可以独立扩展推理或训练资源，而无需重启整个实验。
...
在训练侧，AReaL 提供三种引擎选择：Megatron 引擎在大规模 MoE 训练上久经考验、功能完备；FSDP 引擎基于 HuggingFace
模型、兼容性最广（如VLM）；Archon 引擎 PyTorch 原生、安装最简。在推理侧，SGLang 或 vLLM 不管你熟悉哪一个都可以快速切换。

算法表现上，AReaL v1.0.0 能够用 48 张 H200 卡训练 235B 参数的 Qwen3-MoE 模型，并在 Tau²-bench 上取得了超越 Gemini
3.0 Pro 和 GPT 5.2 的 SOTA 表现。
...
前文概述了 AReaL 要解决的三大痛点（训推耦合、Agent 接入成本、代码维护难度）以及对应的解决方案。接下来，让我们深入探索 v1.0.0
版本如何将这些理念落地为具体的技术实现。
...
树状注意力带来了显著的性能提升，单 Worker 训练吞吐最高提升 **8.31x**，集群整体吞吐最高提升 **6.20x**，相比于基线方案减少超过了 50% 的 GPU
显存占用。
...
更令人兴奋的是，我们也将这些驾驭 AI coding "武功秘籍"**完全开源**，让每位开发者都能借助"专业团队"，在 AReaL 中加速自己的 Agent RL 应用开发。
...
- **Archon 引擎生产级效率优化**：更完善的功能支持（如LoRA）、更高效的通信和并行优化
- **可用性提升**：让AReaL中的系统组件灵活拼接、即插即用，不再需要复杂的配置
- **VLM/Omini模型Agent训练**：集成更多模态Agentic的强化学习训练
- **AI 辅助开发能力持续进化**：让 AI 专家团队更懂AReaL，也更懂你的需求
