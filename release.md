...
| -------- | ------------------------------------- | -------------------- |
| 安装复杂 | Docker + transformer_engine/apex 编译 | **需要安装torch**    |
...
对于常见的开发任务，我们提供了分步骤的引导式工作流：

| 技能                 | 用途              | 效果                           |
| -------------------- | ----------------- | ------------------------------ |
| `/add-dataset`       | 添加新数据集      | 自动创建文件、注册、生成测试   |
| `/add-workflow`      | 创建 Agent 工作流 | 引导实现完整的 RolloutWorkflow |
| `/add-reward`        | 实现奖励函数      | 遵循 AReaL 约定，避免常见错误  |
| `/debug-distributed` | 排查分布式问题    | 系统化排查 hang/OOM/NCCL 错误  |

### 自动化 Commands

日常开发中的重复性工作，交给自动化命令：

- `/create-pr`：自动 rebase、squash commits、生成 PR 描述
- `/gen-commit-msg`：从 staged changes 智能生成 commit 信息
- `/pr-review`：多维度代码审查，自动检测风险等级

## 典型开发流程

以下是使用 AI 辅助开发在 AReaL 中添加新功能的典型流程：
...
这套 AI 辅助开发配置不仅用于 AReaL 自身的开发，**你也可以直接使用它来加速自己的 Agent RL 应用开发**：

1. **快速上手**：使用 `/add-workflow` 创建自定义 Agent 工作流
1. **实现奖励**：使用 `/add-reward` 添加特定领域的奖励函数
1. **排查问题**：使用 `/debug-distributed` 系统化排查分布式训练问题
1. **算法指导**：借助 `algorithm-expert` 获取 GRPO/PPO/DAPO 算法建议
